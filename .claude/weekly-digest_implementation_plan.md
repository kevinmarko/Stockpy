...
|---|---|---|---|---|
| 0 | 1 | **WP-0 (Scaffold)** — the `DigestPayload` contract (5 slots, each nullable, each tagged with its selection reason and confidence tier from §4's fallback ladder) | New type/contract file `pilots/digest_models.py` | — |
| 1 | A | **WP-A (View-tracking log)** — a minimal, own-`Base`/table/`session_scope` store recording `(symbol, viewed_at)` | `data/symbol_view_store.py` | WP-0 |
| 1 | B | **WP-B (Sector-gap composition)** — reads current paper holdings + tracked universe's sector fields, computes underrepresented sectors | New composition function | WP-0 |
| 1 | D | **WP-D (Scheduling skeleton)** — stub weekly trigger, no digest content yet, proves the trigger fires reliably | `desktop/daemon_runtime.py` | WP-0 |
