...
tools: [view_file, search_directory, find_file, run_command]
subagent: true
model: pro
---
...
Ported from this repo's Claude Code sibling agent (`.claude/agents/api-parity-reviewer.md`)
to Antigravity's subagent format. `model: pro` is set explicitly here, per the operator's
own checklist: cross-referencing `liveApi`/`mockApi`/`types.ts` shape parity, tracing
`http<T>` generic usage, and grepping every screen/component for real call sites to find an
orphaned endpoint is genuinely hard multi-file reasoning -- the same bar CLAUDE.md sets for
pinning a subagent to the higher-tier reasoning model rather than a lighter default. This is
exactly the class of bug the agent exists to catch: `api.getOptions()` sat fully wired for
an entire feature's lifetime with zero callers before anyone noticed.

Tool names are mapped from the Claude original's `tools: Read, Grep, Glob, Bash` to
Antigravity's equivalents (`view_file`, `search_directory`, `find_file`, `run_command`),
mirroring the mapping `.agents/agents/test-writer.md` already established for its own tool
list. `run_command` is required here (unlike `honesty-auditor`) because this agent actually
executes `npm run typecheck` and reports the real result rather than eyeballing it.
