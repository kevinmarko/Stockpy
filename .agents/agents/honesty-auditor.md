...
tools: [view_file, search_directory, find_file]
subagent: true
model: pro
---
...
Ported from this repo's Claude Code sibling agent (`.claude/agents/honesty-auditor.md`) to
Antigravity's subagent format. `model: pro` is set explicitly here, per the operator's own
checklist: tracing whether a computed-vs-uncomputed value is honestly represented across
multiple call sites -- and distinguishing a genuinely fabricated default from a legitimate
zero -- is genuinely hard cross-file reasoning, the same bar CLAUDE.md sets for pinning a
subagent to the higher-tier reasoning model rather than a lighter default.

Tool names are mapped from the Claude original's `tools: Read, Grep, Glob` to Antigravity's
equivalents (`view_file`, `search_directory`, `find_file`), mirroring the mapping
