...
6. **Build the vendor bundle if you haven't already** (one-time per machine,
   ...
   time.
7. **Add a test.** Follow `tests/test_investyo_mcp_widgets.py`'s pattern:
   `TestRenderWidgetHtml`-style tests build fixtures entirely inside
   `tmp_path` via `monkeypatch.setattr` on `mcp_widget_resources.BUNDLE_PATH`
   / `TEMPLATES_DIR` (so they don't depend on the real npm build having run
   in CI), while `TestToolMetaWiringConsistency`-style tests assert against
   the real `investyo_mcp_server.py` module-level `_WIDGETS_AVAILABLE`/
   `_MY_WIDGET_UI` constants and that adding `meta` didn't change the tool's
   text output.
