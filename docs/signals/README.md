...
| [`sector_quality_rank`](sector_quality_rank.md) | 15.0 | `signals/sector_quality_rank.py` | Sloan accrual quality + Novy-Marx gross profitability, ranked within sector (dormant — raw inputs not yet wired into `universe_df`, contributes 0.0 until a follow-up data-plumbing task; see doc's Data Availability Gap) | — (dormant; no Pilot until real data is wired in) | — |
...
| [`lstm_attention_forecast`](google_trends_asvi.md) | 0.0 | `signals/lstm_attention_forecast.py` | Google Trends ASVI LSTM-Attention Forecast (Phase 5) | Google Trends LSTM (`lstm-attention-forecast`) | — (API rate limits prevent historical point-in-time backtest) |
