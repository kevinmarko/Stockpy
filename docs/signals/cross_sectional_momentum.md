...
- **Feature-Compatibility Caveat**: RESOLVED — the live per-ticker row (`strategy_engine.py::evaluate_security()`) was widened with the 9 previously-missing feature names this signal (and its 5 siblings) declare, genuinely computed in `processing_engine.py`/`ml/forecast_backfill.py`, not merely whitelisted. `check_feature_compatibility()` now returns `True` for this signal's real `meta_label_features` (see `tests/test_train_meta_labelers.py::TestSixEligibleSignalsFeatureCompatibility`). A trained model for this signal can now actually reach `global_meta_registry` once it separately clears the DSR/PBO deployability gate (unchanged by this fix). **Separate, still-open, pre-existing gap**: this signal's own Signal column is unconditionally NaN in the backfill screen's dummy `SignalContext` (per `ml/forecast_backfill.py`'s "KNOWN GAP" docstring on `step_3_generate_primary_signals`), so it likely still never produces a trained model to register in the first place — a different, unfixed problem from the feature-compatibility gate this change closed.
