...
`STRATEGY_REGISTRY["cross_sectional_momentum"]`'s universe changed from a hardcoded
...
_load_wide_universe()` loader. `cross_sectional_momentum`'s per-ticker computation
collapses into date-indexed columns before CPCV ever sees the data, so this adapter's
CPCV cost is `O(dates)` regardless of ticker count — the widened universe was free to
apply here.

| Metric | Before (30-name list) | After (504-name real roster) | Gate |
|---|---|---|---|
| Sharpe | 0.675 | **0.995** | > 0.50 ✅ |
| PBO | 0.000 | 0.492 | < 0.50 ✅ (narrow margin — see note) |
| DSR | 0.999 | 1.000 | > 0.95 ✅ |
| MaxDD | 29.3% | **25.0%** | < 30% ✅ |
| `deployable` | True | **True** (unchanged) | |

`PBO` moved from 0.000 to 0.492 in the same run that `n_trials` moved from 1 to 2 (the
wider universe changed which variants the adapter's own trial-count bookkeeping sees) —
still under the `< 0.50` gate, but close enough to be worth watching on a future re-run
rather than treated as settled. The "before" row above is one of four
`cross_sectional_momentum`/`relative_strength_xsec`/`multifactor_lowvol_size`/
`signal_replay_balanced_blend` rows that reported near-identical Sharpe/PBO/DSR/MaxDD to
5-6 significant figures in the baseline capture for this change — flagged as a possible
harness-invocation bug in `docs/VALIDATION_STRATEGY_FIX_LOG.md`'s 2026-08-21 entry,
unresolved as of this writing; read the "before" column with that caveat.
...
- **Feature-Compatibility Caveat**: RESOLVED — the live per-ticker row (`strategy_engine.py::evaluate_security()`) was widened with the 9 previously-missing feature names this signal (and its 5 siblings) declare, genuinely computed in `processing_engine.py`/`ml/forecast_backfill.py`, not merely whitelisted. `check_feature_compatibility()` now returns `True` for this signal's real `meta_label_features` (see `tests/test_train_meta_labelers.py::TestSixEligibleSignalsFeatureCompatibility`). A trained model for this signal can now actually reach `global_meta_registry` once it separately clears the DSR/PBO deployability gate (unchanged by this fix). **Correction: the "Signal column unconditionally NaN" gap this section previously described is stale and no longer accurate.** `step_3_generate_primary_signals` now detects this module's overridden `pre_compute` and routes it through `_run_cross_sectional_module`'s real two-phase replay (via `compute_batch_xsec`'s vectorized fast path) — verified directly: a synthetic-panel run trains all 4 horizons with a genuinely two-sided (+1/-1) Signal column. The dangling "KNOWN GAP docstring" this section referenced was itself removed from `ml/forecast_backfill.py` by an earlier fix; do not treat that docstring as still describing current behavior.
