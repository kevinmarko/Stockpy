...
<<<<<<< HEAD
- Measured at commit: `0e420ed64f6d6c008afbea729f922801d16a90d4`
=======
- Measured at commit: `89e450ae44076727f5ae6e2217a4642dd93797b6`
>>>>>>> origin/main
...
<<<<<<< HEAD
`len(Settings.model_fields)` = **385**
=======
`len(Settings.model_fields)` = **391**
>>>>>>> origin/main
...
Fields whose name ends in `_ENABLED`: **101**
...
|---|---|---|---|
<<<<<<< HEAD
| `ALLOWED_KEYS` | 334 | 334 | 0 duplicate entries (clean) |
=======
| `ALLOWED_KEYS` | 340 | 340 | 0 duplicate entries (clean) |
>>>>>>> origin/main
| `SECRET_KEYS` | 44 | 44 | 0 duplicate entries |
...
|---|---|---|
| `SECRET` | 42 | in `env_io.SECRET_KEYS` |
<<<<<<< HEAD
| `IN_ALLOWED_KEYS` | 334 | in `env_io.ALLOWED_KEYS` |
=======
| `IN_ALLOWED_KEYS` | 340 | in `env_io.ALLOWED_KEYS` |
>>>>>>> origin/main
...
|---|---|---|---|
<<<<<<< HEAD
| `ALERT_FILE_PATH` | L1359 | yes | Absolute path for JSON-lines alert log file. None = disabled. |
| `GCLOUD_BIN` | L4622 | yes | Path to the gcloud binary for environment integrations. |
| `GRAVITY_AI_RUNNER_OUTPUT_PATH` | L3908 | yes | Where the runner writes the per-step Claude + Gemini verdicts. Lives under output/ which is gitignored. |
| `LLM_COMMENTARY_CACHE_PATH` | L3793 | yes | JSON cache for LLM commentary results. Day-bucketed; safe to delete manually. Lives under output/ which is gitignored. |
| `LOCAL_DATA_ROOT` | L1608 | yes | Machine-global root for ALL locally-generated model/data artifacts (trained models, SQLite DBs, caches, logs) -- lives OUTSIDE every git worktree/checkout on purpose. This repo runs many worktrees ... |
| `OUTPUT_DIR` | L1625 | yes | Directory for generated reports. Defaults to <LOCAL_DATA_ROOT>/output when unset. |
| `PROMPT_CACHE_DIR` | L4060 | yes | Directory for the signed-version disk cache. Each prompt ID gets a sub-directory; up to PROMPT_CACHE_KEEP_VERSIONS signed .json files are kept per ID for offline rollback. |
| `SYNC_WATCHLIST_FILES` | L1630 | yes | Colon-separated paths (shell PATH convention) to additional plain-text watchlist files (one ticker per line, '#' = comment) consumed by data.robinhood_client.discover_universe(). Missing files are ... |
| `WATCH_RULES_FILE` | L3678 | yes | Path to watch_rules.yaml. Defines per-symbol ntfy push-alert rules (action_change, conviction_above, conviction_below). Missing file = no rules active (silent no-op). |
=======
...
|---|---|---|---|---|
<<<<<<< HEAD
| `FMP_API_KEY` | `settings.py:694` | no | yes | yes |
| `FOLLOW_API_TOKEN` | `settings.py:332` | no | yes | yes |
| `JULES_API_KEY` | `settings.py:502` | no | yes | yes |
| `MCP_HTTP_BEARER_TOKEN` | `settings.py:346` | no | yes | yes |
| `MCP_OAUTH_PASSWORD` | `settings.py:394` | no | yes | yes |
| `ORCHESTRATOR_DAEMON_TOKEN` | `settings.py:313` | no | yes | yes |
| `PROMPT_REGISTRY_PUBLISH_TOKEN` | `settings.py:4025` | no | yes | yes |
| `PROMPT_REGISTRY_SIGNING_KEY` | `settings.py:4033` | no | yes | yes |
| `PROMPT_REGISTRY_TOKEN` | `settings.py:4017` | no | yes | yes |
| `PROMPT_REGISTRY_URL` | `settings.py:4009` | no | yes | yes |
| `STATE_API_TOKEN` | `settings.py:305` | no | yes | yes |
...
- `PUT`/`POST`/`PATCH`/`DELETE` routes total: **67**
>>>>>>> origin/main
...
|---|---|---|---|---|---|---|---|
| `/observability/macro-gate` | PUT | `put_macro_gate` | 1898 | yes | no | no | `next_daemon_restart` |
| `/llm/setting` | PUT | `set_llm_setting` | 3096 | yes | no | no | `immediately`, `next_daemon_restart` |
| `/automation/schedule/interval` | PUT | `set_automation_interval` | 3729 | yes | no | yes | `immediately`, `next_daemon_restart` |
| `/strategy/modules` | PUT | `set_strategy_modules` | 3812 | yes | no | no | `next_daemon_restart` |
| `/automation/execution-mode` | PUT | `update_execution_mode` | 3899 | yes | no | no | `next_daemon_restart` |
| `/settings/tunables` | PUT | `put_settings_tunables` | 4334 | yes | no | no | _(none)_ |
| `/settings/tunables` | PATCH | `put_settings_tunables` | 4334 | yes | no | no | _(none)_ |
| `/settings/sentiment` | PUT | `put_settings_sentiment` | 4986 | yes | no | no | _(none)_ |
| `/settings/sentiment` | PATCH | `put_settings_sentiment` | 4986 | yes | no | no | _(none)_ |
| `/settings/sector-selection` | PUT | `put_settings_sector_selection` | 5011 | yes | no | no | _(none)_ |
| `/settings/sector-selection` | PATCH | `put_settings_sector_selection` | 5011 | yes | no | no | _(none)_ |
| `/settings/cache-long-short` | PUT | `put_settings_cache_long_short` | 5036 | yes | no | no | _(none)_ |
| `/settings/cache-long-short` | PATCH | `put_settings_cache_long_short` | 5036 | yes | no | no | _(none)_ |
| `/settings/paper-broker` | PUT | `put_settings_paper_broker` | 5058 | yes | no | no | _(none)_ |
| `/settings/paper-broker` | PATCH | `put_settings_paper_broker` | 5058 | yes | no | no | _(none)_ |
| `/settings/feature-flags` | PUT | `put_feature_flags_settings` | 5122 | yes | no | no | _(none)_ |
| `/settings/feature-flags` | PATCH | `put_feature_flags_settings` | 5122 | yes | no | no | _(none)_ |
| `/settings/fmp` | PUT | `put_settings_fmp` | 5151 | yes | no | no | _(none)_ |
| `/settings/fmp` | PATCH | `put_settings_fmp` | 5151 | yes | no | no | _(none)_ |
| `/settings/etf-transmission` | PUT | `put_settings_etf_transmission` | 5176 | yes | no | no | _(none)_ |
| `/settings/etf-transmission` | PATCH | `put_settings_etf_transmission` | 5176 | yes | no | no | _(none)_ |
| `/prompts/pin` | PUT | `put_prompts_pin` | 5447 | yes | no | no | `next_daemon_restart` |
...
<<<<<<< HEAD
Scope: **413** production `.py` files (excludes `tests/`, `test_*.py`, `conftest.py`, `.venv/`, `webapp/`, `node_modules/`).
=======
Scope: **401** production `.py` files (excludes `tests/`, `test_*.py`, `conftest.py`, `.venv/`, `webapp/`, `node_modules/`).
>>>>>>> origin/main
...
|---|---|---|
<<<<<<< HEAD
| (a) `settings.KEY` | 724 | 228 |
| (b) `getattr(settings, "KEY", default)` | 306 | 179 |
| (c) `getattr(settings, <var>)` (dynamic) | 17 sites | n/a — key not statically known |
| (d) `os.environ` / `os.getenv("KEY")` | 24 | 17 |

Fields reached by at least one form: **374** of 385.

### Fields with NO statically-attributable read — **11**
=======
| (a) `settings.KEY` | 730 | 231 |
| (b) `getattr(settings, "KEY", default)` | 313 | 183 |
| (c) `getattr(settings, <var>)` (dynamic) | 17 sites | n/a — key not statically known |
| (d) `os.environ` / `os.getenv("KEY")` | 24 | 17 |

Fields reached by at least one form: **381** of 391.

### Fields with NO statically-attributable read — **10**
>>>>>>> origin/main
...
|---|---|---|
| `AI_GENERATION_API_ENABLED` | `api/data_api.py:188`, `settings_keysets.py:359` | likely read dynamically |
| `EDGAR_FULLTEXT_CHUNK_TOKENS` | `api/pilots_api.py:4753` | likely read dynamically |
| `EDGAR_FULLTEXT_FORMS` | `api/pilots_api.py:4752` | likely read dynamically |
| `ETF_HOLDINGS_TICKERS` | `api/pilots_api.py:4922`, `gui/panels/settings_manager.py:126` | likely read dynamically |
| `FMP_ECON_INDICATORS` | `api/pilots_api.py:4894`, `gui/panels/settings_manager.py:162` | likely read dynamically |
<<<<<<< HEAD
| `OPTIONS_0DTE_ENABLED` | _none_ | no read and no name reference found |
...
| `PROMPT_REGISTRY_REFRESH_SECONDS` | `Gravity AI Review Suite.py:11027` | likely read dynamically |
...
### Fields reachable ONLY via form (b) or (d), never via (a) — **150**
>>>>>>> origin/main
...
<<<<<<< HEAD
| `OPTIONS_0DTE_HARD_EXIT_TIME` | b | 3 | 0 |
| `OPTIONS_0DTE_PROFIT_TARGET_PCT` | b | 2 | 0 |
| `OPTIONS_0DTE_STOP_LOSS_PCT` | b | 2 | 0 |
=======
...
| `api/pilots_api.py:3165` | `getattr(settings, body.key)` |
| `api/pilots_api.py:4271` | `getattr(settings, key, None)` |
| `api/pilots_api.py:4376` | `getattr(settings, key, None)` |
