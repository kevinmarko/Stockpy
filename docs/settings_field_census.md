...
- Measured at commit: `8df8b50a74b72f4d01e620721b53731b1dd8e699`
...
Fields whose name ends in `_ENABLED`: **88**
...
|---|---|---|---|
| `ALLOWED_KEYS` | 306 | 306 | 0 duplicate entries (clean) |
...
| `IN_ALLOWED_KEYS` | 306 | in `env_io.ALLOWED_KEYS` |
...
| `GCLOUD_BIN` | L4413 | yes | Path to the gcloud binary for environment integrations. |
| `GRAVITY_AI_RUNNER_OUTPUT_PATH` | L3718 | yes | Where the runner writes the per-step Claude + Gemini verdicts. Lives under output/ which is gitignored. |
| `LLM_COMMENTARY_CACHE_PATH` | L3603 | yes | JSON cache for LLM commentary results. Day-bucketed; safe to delete manually. Lives under output/ which is gitignored. |
| `OUTPUT_DIR` | L1475 | yes | Directory for generated reports. |
| `PROMPT_CACHE_DIR` | L3870 | yes | Directory for the signed-version disk cache. Each prompt ID gets a sub-directory; up to PROMPT_CACHE_KEEP_VERSIONS signed .json files are kept per ID for offline rollback. |
...
| `WATCH_RULES_FILE` | L3488 | yes | Path to watch_rules.yaml. Defines per-symbol ntfy push-alert rules (action_change, conviction_above, conviction_below). Missing file = no rules active (silent no-op). |
...
| `PROMPT_REGISTRY_PUBLISH_TOKEN` | `settings.py:3835` | no | yes | yes |
| `PROMPT_REGISTRY_SIGNING_KEY` | `settings.py:3843` | no | yes | yes |
| `PROMPT_REGISTRY_TOKEN` | `settings.py:3827` | no | yes | yes |
| `PROMPT_REGISTRY_URL` | `settings.py:3819` | no | yes | yes |
...
|---|---|---|---|---|---|---|---|
| `/observability/macro-gate` | PUT | `put_macro_gate` | 1853 | yes | no | no | `next_daemon_restart` |
| `/llm/setting` | PUT | `set_llm_setting` | 3049 | yes | no | no | `immediately`, `next_daemon_restart` |
| `/automation/schedule/interval` | PUT | `set_automation_interval` | 3682 | yes | no | yes | `immediately`, `next_daemon_restart` |
| `/strategy/modules` | PUT | `set_strategy_modules` | 3765 | yes | no | no | `next_daemon_restart` |
| `/automation/execution-mode` | PUT | `update_execution_mode` | 3852 | yes | no | no | `next_daemon_restart` |
| `/settings/tunables` | PUT | `put_settings_tunables` | 4227 | yes | no | no | _(none)_ |
| `/settings/tunables` | PATCH | `put_settings_tunables` | 4227 | yes | no | no | _(none)_ |
| `/settings/sentiment` | PUT | `put_settings_sentiment` | 4854 | yes | no | no | _(none)_ |
| `/settings/sentiment` | PATCH | `put_settings_sentiment` | 4854 | yes | no | no | _(none)_ |
| `/settings/sector-selection` | PUT | `put_settings_sector_selection` | 4879 | yes | no | no | _(none)_ |
| `/settings/sector-selection` | PATCH | `put_settings_sector_selection` | 4879 | yes | no | no | _(none)_ |
| `/settings/cache-long-short` | PUT | `put_settings_cache_long_short` | 4904 | yes | no | no | _(none)_ |
| `/settings/cache-long-short` | PATCH | `put_settings_cache_long_short` | 4904 | yes | no | no | _(none)_ |
| `/settings/feature-flags` | PUT | `put_feature_flags_settings` | 4968 | yes | no | no | _(none)_ |
| `/settings/feature-flags` | PATCH | `put_feature_flags_settings` | 4968 | yes | no | no | _(none)_ |
| `/settings/fmp` | PUT | `put_settings_fmp` | 4997 | yes | no | no | _(none)_ |
| `/settings/fmp` | PATCH | `put_settings_fmp` | 4997 | yes | no | no | _(none)_ |
| `/settings/etf-transmission` | PUT | `put_settings_etf_transmission` | 5022 | yes | no | no | _(none)_ |
| `/settings/etf-transmission` | PATCH | `put_settings_etf_transmission` | 5022 | yes | no | no | _(none)_ |
| `/prompts/pin` | PUT | `put_prompts_pin` | 5293 | yes | no | no | `next_daemon_restart` |
...
Fields reached by at least one form: **345** of 354.
...
| `EDGAR_FULLTEXT_CHUNK_TOKENS` | `api/pilots_api.py:4646` | likely read dynamically |
| `EDGAR_FULLTEXT_FORMS` | `api/pilots_api.py:4645` | likely read dynamically |
| `ETF_HOLDINGS_TICKERS` | `api/pilots_api.py:4790`, `gui/panels/settings_manager.py:126` | likely read dynamically |
| `FMP_ECON_INDICATORS` | `api/pilots_api.py:4770`, `gui/panels/settings_manager.py:162` | likely read dynamically |
...
| `api/pilots_api.py:3118` | `getattr(settings, body.key)` |
| `api/pilots_api.py:4164` | `getattr(settings, key, None)` |
| `api/pilots_api.py:4269` | `getattr(settings, key, None)` |
