...
- Measured at commit: `cc847af1a8fab27d30f3a246dfc8d2e357678afa`
...
Fields whose name ends in `_ENABLED`: **91**
...
| `GCLOUD_BIN` | L4536 | yes | Path to the gcloud binary for environment integrations. |
| `GRAVITY_AI_RUNNER_OUTPUT_PATH` | L3830 | yes | Where the runner writes the per-step Claude + Gemini verdicts. Lives under output/ which is gitignored. |
| `LLM_COMMENTARY_CACHE_PATH` | L3715 | yes | JSON cache for LLM commentary results. Day-bucketed; safe to delete manually. Lives under output/ which is gitignored. |
| `OUTPUT_DIR` | L1552 | yes | Directory for generated reports. |
| `PROMPT_CACHE_DIR` | L3982 | yes | Directory for the signed-version disk cache. Each prompt ID gets a sub-directory; up to PROMPT_CACHE_KEEP_VERSIONS signed .json files are kept per ID for offline rollback. |
...
| `WATCH_RULES_FILE` | L3600 | yes | Path to watch_rules.yaml. Defines per-symbol ntfy push-alert rules (action_change, conviction_above, conviction_below). Missing file = no rules active (silent no-op). |
...
| `PROMPT_REGISTRY_PUBLISH_TOKEN` | `settings.py:3947` | no | yes | yes |
| `PROMPT_REGISTRY_SIGNING_KEY` | `settings.py:3955` | no | yes | yes |
| `PROMPT_REGISTRY_TOKEN` | `settings.py:3939` | no | yes | yes |
| `PROMPT_REGISTRY_URL` | `settings.py:3931` | no | yes | yes |
...
| `/settings/tunables` | PUT | `put_settings_tunables` | 4261 | yes | no | no | _(none)_ |
| `/settings/tunables` | PATCH | `put_settings_tunables` | 4261 | yes | no | no | _(none)_ |
| `/settings/sentiment` | PUT | `put_settings_sentiment` | 4889 | yes | no | no | _(none)_ |
| `/settings/sentiment` | PATCH | `put_settings_sentiment` | 4889 | yes | no | no | _(none)_ |
| `/settings/sector-selection` | PUT | `put_settings_sector_selection` | 4914 | yes | no | no | _(none)_ |
| `/settings/sector-selection` | PATCH | `put_settings_sector_selection` | 4914 | yes | no | no | _(none)_ |
| `/settings/cache-long-short` | PUT | `put_settings_cache_long_short` | 4939 | yes | no | no | _(none)_ |
| `/settings/cache-long-short` | PATCH | `put_settings_cache_long_short` | 4939 | yes | no | no | _(none)_ |
...
| `/settings/etf-transmission` | PUT | `put_settings_etf_transmission` | 5057 | yes | no | no | _(none)_ |
| `/settings/etf-transmission` | PATCH | `put_settings_etf_transmission` | 5057 | yes | no | no | _(none)_ |
| `/prompts/pin` | PUT | `put_prompts_pin` | 5328 | yes | no | no | `next_daemon_restart` |
...
Scope: **369** production `.py` files (excludes `tests/`, `test_*.py`, `conftest.py`, `.venv/`, `webapp/`, `node_modules/`).
...
|---|---|---|
| `AI_GENERATION_API_ENABLED` | `api/data_api.py:188`, `settings_keysets.py:316` | likely read dynamically |
| `EDGAR_FULLTEXT_CHUNK_TOKENS` | `api/pilots_api.py:4680` | likely read dynamically |
| `EDGAR_FULLTEXT_FORMS` | `api/pilots_api.py:4679` | likely read dynamically |
| `ETF_HOLDINGS_TICKERS` | `api/pilots_api.py:4825`, `gui/panels/settings_manager.py:126` | likely read dynamically |
| `FMP_ECON_INDICATORS` | `api/pilots_api.py:4804`, `gui/panels/settings_manager.py:162` | likely read dynamically |
...
| `api/pilots_api.py:4198` | `getattr(settings, key, None)` |
