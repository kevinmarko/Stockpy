...
- Measured at commit: `5dab7e76d2a09afc3804b2f5afd88445f1e5df28`
...
|---|---|---|---|
| `ALLOWED_KEYS` | 378 | 378 | 0 duplicate entries (clean) |
...
| `IN_ALLOWED_KEYS` | 378 | in `env_io.ALLOWED_KEYS` |
...
|---|---|---|---|
| `ALERT_FILE_PATH` | L1574 | yes | Absolute path for JSON-lines alert log file. None = disabled. |
| `GCLOUD_BIN` | L4980 | yes | Path to the gcloud binary for environment integrations. |
| `GRAVITY_AI_RUNNER_OUTPUT_PATH` | L4214 | yes | Where the runner writes the per-step Claude + Gemini verdicts. Lives under output/ which is gitignored. |
| `LLM_COMMENTARY_CACHE_PATH` | L4039 | yes | JSON cache for LLM commentary results. Day-bucketed; safe to delete manually. Lives under output/ which is gitignored. |
| `LOCAL_DATA_ROOT` | L1827 | yes | Machine-global root for ALL locally-generated model/data artifacts (trained models, SQLite DBs, caches, logs) -- lives OUTSIDE every git worktree/checkout on purpose. This repo runs many worktrees ... |
| `OUTPUT_DIR` | L1844 | yes | Directory for generated reports. Defaults to <LOCAL_DATA_ROOT>/output when unset. |
| `PROMPT_CACHE_DIR` | L4366 | yes | Directory for the signed-version disk cache. Each prompt ID gets a sub-directory; up to PROMPT_CACHE_KEEP_VERSIONS signed .json files are kept per ID for offline rollback. |
| `SYNC_WATCHLIST_FILES` | L1857 | yes | Colon-separated paths (shell PATH convention) to additional plain-text watchlist files (one ticker per line, '#' = comment) consumed by data.robinhood_client.discover_universe(). Missing files are ... |
| `WATCH_RULES_FILE` | L3924 | yes | Path to watch_rules.yaml. Defines per-symbol ntfy push-alert rules (action_change, conviction_above, conviction_below). Missing file = no rules active (silent no-op). |
...
|---|---|---|---|---|
| `FMP_API_KEY` | `settings.py:806` | no | yes | yes |
| `FOLLOW_API_TOKEN` | `settings.py:444` | no | yes | yes |
| `JULES_API_KEY` | `settings.py:614` | no | yes | yes |
| `MCP_HTTP_BEARER_TOKEN` | `settings.py:458` | no | yes | yes |
| `MCP_OAUTH_PASSWORD` | `settings.py:506` | no | yes | yes |
| `ORCHESTRATOR_DAEMON_TOKEN` | `settings.py:425` | no | yes | yes |
| `PROMPT_REGISTRY_PUBLISH_TOKEN` | `settings.py:4331` | no | yes | yes |
| `PROMPT_REGISTRY_SIGNING_KEY` | `settings.py:4339` | no | yes | yes |
| `PROMPT_REGISTRY_TOKEN` | `settings.py:4323` | no | yes | yes |
| `PROMPT_REGISTRY_URL` | `settings.py:4315` | no | yes | yes |
| `STATE_API_TOKEN` | `settings.py:417` | no | yes | yes |

## 6. Live-write endpoint inventory — `api/pilots_api.py`

- `PUT`/`POST`/`PATCH`/`DELETE` routes total: **85**
...
Scope: **437** production `.py` files (excludes `tests/`, `test_*.py`, `conftest.py`, `.venv/`, `webapp/`, `node_modules/`).
...
Fields reached by at least one form: **418** of 430.
...
| `EXPERIMENTS_ENABLED` | _none_ | no read and no name reference found |
| `EXPERIMENT_DEFAULT_MIN_SAMPLES` | _none_ | no read and no name reference found |
| `EXPERIMENT_MAX_CONCURRENT` | _none_ | no read and no name reference found |
...
| `UNIVERSE_SYNC_ENABLED` | `api/data_api.py:1385`, `pilots/feature_flags.py:49` | likely read dynamically |
