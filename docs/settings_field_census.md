...
- Measured at commit: `6c0502da1d8c99297e01450f3f9f5bd0e420c1a6`
...
Fields whose name ends in `_ENABLED`: **95**
...
|---|---|---|---|
| `ALLOWED_KEYS` | 316 | 316 | 0 duplicate entries (clean) |
...
| `IN_ALLOWED_KEYS` | 316 | in `env_io.ALLOWED_KEYS` |
...
|---|---|---|---|
| `ALERT_FILE_PATH` | L1280 | yes | Absolute path for JSON-lines alert log file. None = disabled. |
| `GCLOUD_BIN` | L4540 | yes | Path to the gcloud binary for environment integrations. |
| `GRAVITY_AI_RUNNER_OUTPUT_PATH` | L3826 | yes | Where the runner writes the per-step Claude + Gemini verdicts. Lives under output/ which is gitignored. |
| `LLM_COMMENTARY_CACHE_PATH` | L3711 | yes | JSON cache for LLM commentary results. Day-bucketed; safe to delete manually. Lives under output/ which is gitignored. |
| `LOCAL_DATA_ROOT` | L1529 | yes | Machine-global root for ALL locally-generated model/data artifacts (trained models, SQLite DBs, caches, logs) -- lives OUTSIDE every git worktree/checkout on purpose. This repo runs many worktrees ... |
| `OUTPUT_DIR` | L1546 | yes | Directory for generated reports. Defaults to <LOCAL_DATA_ROOT>/output when unset. |
| `PROMPT_CACHE_DIR` | L3978 | yes | Directory for the signed-version disk cache. Each prompt ID gets a sub-directory; up to PROMPT_CACHE_KEEP_VERSIONS signed .json files are kept per ID for offline rollback. |
| `SYNC_WATCHLIST_FILES` | L1551 | yes | Colon-separated paths (shell PATH convention) to additional plain-text watchlist files (one ticker per line, '#' = comment) consumed by data.robinhood_client.discover_universe(). Missing files are ... |
| `WATCH_RULES_FILE` | L3596 | yes | Path to watch_rules.yaml. Defines per-symbol ntfy push-alert rules (action_change, conviction_above, conviction_below). Missing file = no rules active (silent no-op). |
...
|---|---|---|---|---|
| `FMP_API_KEY` | `settings.py:615` | no | yes | yes |
| `FOLLOW_API_TOKEN` | `settings.py:253` | no | yes | yes |
| `JULES_API_KEY` | `settings.py:423` | no | yes | yes |
...
| `PROMPT_REGISTRY_PUBLISH_TOKEN` | `settings.py:3943` | no | yes | yes |
| `PROMPT_REGISTRY_SIGNING_KEY` | `settings.py:3951` | no | yes | yes |
| `PROMPT_REGISTRY_TOKEN` | `settings.py:3935` | no | yes | yes |
| `PROMPT_REGISTRY_URL` | `settings.py:3927` | no | yes | yes |
| `STATE_API_TOKEN` | `settings.py:226` | no | yes | yes |

## 6. Live-write endpoint inventory — `api/pilots_api.py`

- `PUT`/`POST`/`PATCH`/`DELETE` routes total: **50**
...
Scope: **379** production `.py` files (excludes `tests/`, `test_*.py`, `conftest.py`, `.venv/`, `webapp/`, `node_modules/`).
...
|---|---|---|
| (a) `settings.KEY` | 722 | 228 |
| (b) `getattr(settings, "KEY", default)` | 263 | 160 |
| (c) `getattr(settings, <var>)` (dynamic) | 17 sites | n/a — key not statically known |
| (d) `os.environ` / `os.getenv("KEY")` | 24 | 17 |

Fields reached by at least one form: **357** of 366.
