...
- Measured at commit: `b88b0cf9e5fe2d6944c604818eb2966ebacf7180`
...
Fields whose name ends in `_ENABLED`: **113**
...
|---|---|---|---|
| `ALLOWED_KEYS` | 383 | 383 | 0 duplicate entries (clean) |
...
| `IN_ALLOWED_KEYS` | 383 | in `env_io.ALLOWED_KEYS` |
...
|---|---|---|---|
| `ALERT_FILE_PATH` | L1705 | yes | Absolute path for JSON-lines alert log file. None = disabled. |
| `GCLOUD_BIN` | L5111 | yes | Path to the gcloud binary for environment integrations. |
| `GRAVITY_AI_RUNNER_OUTPUT_PATH` | L4345 | yes | Where the runner writes the per-step Claude + Gemini verdicts. Lives under output/ which is gitignored. |
| `LLM_COMMENTARY_CACHE_PATH` | L4170 | yes | JSON cache for LLM commentary results. Day-bucketed; safe to delete manually. Lives under output/ which is gitignored. |
| `LOCAL_DATA_ROOT` | L1958 | yes | Machine-global root for ALL locally-generated model/data artifacts (trained models, SQLite DBs, caches, logs) -- lives OUTSIDE every git worktree/checkout on purpose. This repo runs many worktrees ... |
| `OUTPUT_DIR` | L1975 | yes | Directory for generated reports. Defaults to <LOCAL_DATA_ROOT>/output when unset. |
| `PROMPT_CACHE_DIR` | L4497 | yes | Directory for the signed-version disk cache. Each prompt ID gets a sub-directory; up to PROMPT_CACHE_KEEP_VERSIONS signed .json files are kept per ID for offline rollback. |
| `SYNC_WATCHLIST_FILES` | L1988 | yes | Colon-separated paths (shell PATH convention) to additional plain-text watchlist files (one ticker per line, '#' = comment) consumed by data.robinhood_client.discover_universe(). Missing files are ... |
| `WATCH_RULES_FILE` | L4055 | yes | Path to watch_rules.yaml. Defines per-symbol ntfy push-alert rules (action_change, conviction_above, conviction_below). Missing file = no rules active (silent no-op). |
...
|---|---|---|---|---|
| `FMP_API_KEY` | `settings.py:841` | no | yes | yes |
| `FOLLOW_API_TOKEN` | `settings.py:479` | no | yes | yes |
| `JULES_API_KEY` | `settings.py:649` | no | yes | yes |
| `MCP_HTTP_BEARER_TOKEN` | `settings.py:493` | no | yes | yes |
| `MCP_OAUTH_PASSWORD` | `settings.py:541` | no | yes | yes |
| `ORCHESTRATOR_DAEMON_TOKEN` | `settings.py:460` | no | yes | yes |
| `PROMPT_REGISTRY_PUBLISH_TOKEN` | `settings.py:4462` | no | yes | yes |
| `PROMPT_REGISTRY_SIGNING_KEY` | `settings.py:4470` | no | yes | yes |
| `PROMPT_REGISTRY_TOKEN` | `settings.py:4454` | no | yes | yes |
| `PROMPT_REGISTRY_URL` | `settings.py:4446` | no | yes | yes |
| `STATE_API_TOKEN` | `settings.py:452` | no | yes | yes |
...
|---|---|---|---|---|---|---|---|
| `/observability/macro-gate` | PUT | `put_macro_gate` | 1920 | yes | no | no | `next_daemon_restart` |
| `/llm/setting` | PUT | `set_llm_setting` | 3118 | yes | no | no | `immediately`, `next_daemon_restart` |
| `/automation/schedule/interval` | PUT | `set_automation_interval` | 3751 | yes | no | yes | `immediately`, `next_daemon_restart` |
| `/strategy/modules` | PUT | `set_strategy_modules` | 3834 | yes | no | no | `next_daemon_restart` |
| `/automation/execution-mode` | PUT | `update_execution_mode` | 3921 | yes | no | no | `next_daemon_restart` |
| `/settings/tunables` | PUT | `put_settings_tunables` | 4356 | yes | no | no | _(none)_ |
| `/settings/tunables` | PATCH | `put_settings_tunables` | 4356 | yes | no | no | _(none)_ |
| `/settings/sentiment` | PUT | `put_settings_sentiment` | 5008 | yes | no | no | _(none)_ |
| `/settings/sentiment` | PATCH | `put_settings_sentiment` | 5008 | yes | no | no | _(none)_ |
| `/settings/sector-selection` | PUT | `put_settings_sector_selection` | 5033 | yes | no | no | _(none)_ |
| `/settings/sector-selection` | PATCH | `put_settings_sector_selection` | 5033 | yes | no | no | _(none)_ |
| `/settings/cache-long-short` | PUT | `put_settings_cache_long_short` | 5058 | yes | no | no | _(none)_ |
| `/settings/cache-long-short` | PATCH | `put_settings_cache_long_short` | 5058 | yes | no | no | _(none)_ |
| `/settings/paper-broker` | PUT | `put_settings_paper_broker` | 5080 | yes | no | no | _(none)_ |
| `/settings/paper-broker` | PATCH | `put_settings_paper_broker` | 5080 | yes | no | no | _(none)_ |
| `/settings/feature-flags` | PUT | `put_feature_flags_settings` | 5144 | yes | no | no | _(none)_ |
| `/settings/feature-flags` | PATCH | `put_feature_flags_settings` | 5144 | yes | no | no | _(none)_ |
| `/settings/fmp` | PUT | `put_settings_fmp` | 5173 | yes | no | no | _(none)_ |
| `/settings/fmp` | PATCH | `put_settings_fmp` | 5173 | yes | no | no | _(none)_ |
| `/settings/etf-transmission` | PUT | `put_settings_etf_transmission` | 5198 | yes | no | no | _(none)_ |
| `/settings/etf-transmission` | PATCH | `put_settings_etf_transmission` | 5198 | yes | no | no | _(none)_ |
| `/prompts/pin` | PUT | `put_prompts_pin` | 5469 | yes | no | no | `next_daemon_restart` |
...
Scope: **431** production `.py` files (excludes `tests/`, `test_*.py`, `conftest.py`, `.venv/`, `webapp/`, `node_modules/`).
...
|---|---|---|
| (a) `settings.KEY` | 777 | 252 |
| (b) `getattr(settings, "KEY", default)` | 371 | 208 |
| (c) `getattr(settings, <var>)` (dynamic) | 17 sites | n/a — key not statically known |
| (d) `os.environ` / `os.getenv("KEY")` | 25 | 18 |

Fields reached by at least one form: **426** of 435.
...
|---|---|---|
| `EDGAR_FULLTEXT_CHUNK_TOKENS` | `api/pilots_api.py:4775` | likely read dynamically |
| `EDGAR_FULLTEXT_FORMS` | `api/pilots_api.py:4774` | likely read dynamically |
| `ETF_HOLDINGS_TICKERS` | `api/pilots_api.py:4944`, `gui/panels/settings_manager.py:126` | likely read dynamically |
| `FMP_ECON_INDICATORS` | `api/pilots_api.py:4916`, `gui/panels/settings_manager.py:162` | likely read dynamically |
...
| `api/pilots_api.py:3187` | `getattr(settings, body.key)` |
| `api/pilots_api.py:4293` | `getattr(settings, key, None)` |
