...
- `Gravity AI Review Suite.py` — `step_80_robinhood_execution_bridge_audit` (10 checks; originally added to the now-deleted `gravity/__init__.py` package, migrated into the launcher 2026-07-10): off emits nothing; review never placeable; live+cap+clear-KS allows; kill-switch blocks; cap-unset blocks; drop rules + held-SELL qty; settings default+fail-safe; preflight live-without-cap fails + not auto-skipped; no order defs + main.py wiring + agent skill/command exist.
