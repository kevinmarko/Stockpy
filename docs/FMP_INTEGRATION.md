...
All 25 settings live in `settings.py` under `# --- 25. Financial Modeling Prep (data/fmp_client.py) ---`, are mirrored in `.env.example`, and (except the credential) are GUI-writable via `gui/panels/settings_manager.py`. **`FMP_API_KEY` alone never elects FMP as the active provider for anything.** Quotes/bars need `MARKET_DATA_PROVIDER=fmp` in addition to their own gate; fundamentals need `FUNDAMENTALS_SOURCE=fmp` in addition to `FMP_FUNDAMENTALS_ENABLED`. This two-gate convention (the `STOCKTWITS_ENABLED` precedent) exists specifically so that adding a key to light up one diagnostic feed — say, the analyst consensus columns — can never silently change what every valuation metric in the platform is computed from.
