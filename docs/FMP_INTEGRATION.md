...
All 25 settings live in `settings.py` under `# --- 25. Financial Modeling Prep (data/fmp_client.py) ---`, are mirrored in `.env.example`, and (except the credential) are GUI-writable via `gui/panels/settings_manager.py`. **`FMP_API_KEY` alone never elects FMP as the active provider for anything.** Quotes/bars are gated solely by `MARKET_DATA_PROVIDER=fmp`; fundamentals solely by `FUNDAMENTALS_SOURCE=fmp`. **Known gap:** `FMP_QUOTES_ENABLED`/`FMP_BARS_ENABLED`/`FMP_FUNDAMENTALS_ENABLED` were designed as a second, independent gate on those three (the `STOCKTWITS_ENABLED` two-gate precedent, meant so that adding a key to light up one diagnostic feed could never silently change what every valuation metric is computed from) — but as shipped, the provider-selection code never reads them, so setting `MARKET_DATA_PROVIDER=fmp`/`FUNDAMENTALS_SOURCE=fmp` alone is sufficient today regardless of those three flags. The five diagnostic-feed gates (`FMP_ANALYST_ENABLED` through `FMP_SECTOR_SNAPSHOT_ENABLED`) are each a real, enforced, single gate for their own columns. A future change should either wire the missing check in or remove the three unused settings — until then, treat their presence in `.env`/the GUI as inert.
...
### Master feed gates (8) — all default `False` = complete no-op
