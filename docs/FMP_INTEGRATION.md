...
**Confirmed working on Starter:** `/quote` + `batch-quote`; EOD charts (confirmed back to 2008; a per-request observation, not a single-call completeness guarantee for a multi-decade window — see the Known Risks §6 entry on the 5,000-row cap) + intraday; `key-metrics-ttm`, `ratios-ttm`, `financial-scores` (note: `/income-statement-ttm` is Ultimate/Enterprise-only; `trailingEps` falls back to `ratios_ttm.netIncomePerShareTTM`); `/profile`, `/market-cap`, `/shares-float`, `/peers`; analyst `price-target-consensus`, `analyst-estimates`, `grades-summary`, `ratings-snapshot`; calendars (earnings/dividends/splits, including future-dated rows); `treasury-rates` (full curve, daily), `economic-indicators`, `economics-calendar`; `insider-trading/statistics`; `sector-pe-snapshot` + `sector-performance-snapshot` (date-parameterized); news; technical indicators.
...
- `historical_eod()`'s undocumented ~5,000-row-per-request cap on `/historical-price-eod/{variant}`: a window spanning more than ~19.8 trading years silently returns only the most recent ~5,000 rows and drops the older portion — confirmed live, `from_date='2005-01-01'` returned rows starting `2006-10-05`, dropping ~22 months — with `page`/`limit` silently ignored by this endpoint (unlike `stock_news`, which genuinely paginates via those params). Fixed for the one caller that needs a multi-decade window via `historical_eod_full_range()`, migrated into `scripts/refresh_validations.py` only — `historical_eod()` itself was deliberately left unchanged, so any *future* caller passing a multi-decade window to plain `historical_eod()` instead of `historical_eod_full_range()` will hit this silently again.
