...
**Explicit per-call confirmation.** Every `dispatch_jules_task`/`create-session` invocation requires an explicit `confirm=True` (MCP tool) or `--confirm` (CLI) — omitting it dispatches nothing. This mirrors `shared/orchestrator_runner.py`'s `HIGH_STAKES_COMMANDS` gate that this codebase already uses for other irreversible actions (the global kill-switch toggle, a forced Robinhood re-login).
