...
| 4b Gravity split | ⚠️ reverted 2026-07-10 | #structural-phases | Originally: `GravityAIAuditor` → `gravity/__init__.py`, launcher as a 23-line shim. In practice the shim was never completed — `Gravity AI Review Suite.py` kept its full content and both files were independently edited for two weeks (confirmed via `tests/test_gravity_mirrored_invariants.py`'s docstring, which documents the launcher as *deliberately* non-importable). `gravity/__init__.py` had zero importers anywhere and had drifted to carry 10 audit steps (help explainers, advisory agent, trade signals, Robinhood orders/execution bridge, LLM commentary, AI Gravity runner, AI Insights, Opal research, AI Control Center) the live launcher never got. Resolved by migrating those 10 steps into `Gravity AI Review Suite.py` (renumbered step_75–step_85) and deleting `gravity/__init__.py` outright — single source of truth restored. |
