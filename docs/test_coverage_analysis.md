...
| `webapp/` (Pilots PWA — React/TypeScript, ~separate stack from the Python suite above) | n/a (JS/TS) | **Fully done — 9 test files / 100 tests, up from 1 file (`mock.test.ts`, mock-API-layer only).** This closed in three waves: this branch added `@testing-library/react` + tests for `format.ts`/`FollowModal.tsx`; concurrently on `main` (a different session, PRs #266–#268, reconciled into this branch by rebase) a more complete effort shipped the same infrastructure plus `Marketplace`/`PilotDetail`/`Portfolio`/`client.ts` coverage, live-verified against a real running backend; then, as a follow-up once every other screen had coverage, `Onboarding.tsx` (the wizard) and `onboarding.ts` (the localStorage completion-marker module it depends on) were closed out — `webapp/src/onboarding.test.ts` (13 tests: read/write/complete/reset, every localStorage-throws degradation path) and `webapp/src/screens/Onboarding.test.tsx` (16 tests: all 3 steps, the paper-first/gated-queue copy, the real `apiMeta.mockMode` execution-mode notice, the non-deployable-Pilot name-resolution fallback, Get-started/Skip-for-now completion paths, and the persisted-state contract). Every screen in the app now has a test file. | Medium — this is a real, shippable consumer-facing surface; every screen (not just the money-adjacent ones) now has regression protection. | Closed. |
