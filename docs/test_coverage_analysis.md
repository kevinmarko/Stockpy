...
- **Stray, never-collected root test files.** `test_gravity.py` and `test_mock_abc.py` live at the
  repo root. Because `pytest.ini` pins `testpaths = tests`, they are **never collected** — so
  `test_gravity.py` is the *only* direct test of `gravity/__init__.py` (12,834 lines, the largest
  module in the repo) and it never runs.
...
| `gravity/__init__.py` | 12,834 | **Not collected** — its only direct test (`test_gravity.py`) sits at the repo root and is excluded by `testpaths = tests`. | Medium — largest module; audit/verification logic that runs as a subprocess elsewhere, but its unit-level test never executes. | Move/rehome `test_gravity.py` under `tests/` (or split it), fix imports, and register any slow paths behind `@pytest.mark.slow`. |
...
- **Resolve the stray root files:** rehome `test_gravity.py` (and `test_mock_abc.py`) under `tests/`
  so `gravity/__init__.py` is actually exercised; register `@pytest.mark.slow` in `pytest.ini`.
