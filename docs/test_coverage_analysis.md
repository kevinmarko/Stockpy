...
coverage. Phase 5 work below closed item (2)'s highest-value instance and made real progress on (1) —
but a systematic investigation of (1) also **revised the initial framing**: most of the later
Tier/Task-numbered Gravity steps turn out to be independently covered by dedicated test files already
(the file having zero tests of its own does not mean the behaviors it checks are untested elsewhere);
the genuine risk concentrates in the *older* steps that predate today's per-feature test files. See the
Phase 5 section for the full, evidence-based breakdown.

**Phase 5 implementation status (updated as work lands):** closed so far — the `gravity_audit.py`
pure-helper tests, the `alerting_mcp`/`mcp_remote_adapter` owning suites, `data/robinhood_client.py`
(0%→94%), and `ai_verification_prompts.py` (91%, plus a correction to what that file's
`GravityAIAuditor` class actually does vs. what its docstring claims). Two stray dead root test files
were deleted and one unrelated pre-existing test that silently made a live network call was fixed.
Still open: the `Gravity AI Review Suite.py` step-mirroring investigation for steps 1–58 (the likelier
location of a real gap, per the above), `investyo_mcp_server.py`, the remaining ~18 GUI render-wrapper
panels, `webapp/` component tests, and the CI coverage-floor ratchet — see the Phase 5 section for
exact status per item.
...
| **`Gravity AI Review Suite.py`** | **~14,900** | **Effectively 0% by direct execution, but this overstates the real gap** (revised in Phase 5). The file has **zero `def test_` functions of its own** and (space-containing filename) is architecturally excluded from ordinary pytest collection. A systematic investigation of its 20 `step_61`–`step_91` methods (the later Tier/Task-numbered audits) found **every one already independently covered** by a dedicated `tests/test_*.py` file exercising the same production code with equal-or-stricter assertions — e.g. `step_78_advisory_agent_audit` ↔ `tests/test_advisory_agent.py`, `step_89_rolling_beta_lookahead_audit` ↔ `tests/test_indicators_lookahead.py`. Only one narrow, low-priority sub-check (step_72's `CANONICAL_REGIMES` cross-model comparison — see Phase 5 item 1 for detail) was a genuine, unmirrored gap in that batch. The earlier `step_1`–`step_58` inline-block audits (older, predate most per-feature test files) are **not yet investigated** and are where `tests/test_gravity_mirrored_invariants.py`'s own precedent (2 genuine gaps found in that exact layer) suggests real gaps are more likely to live. | **Revised from Highest to Medium** for the already-investigated 61–91 batch (real behavioral coverage exists, just previously unverified/undocumented as such); **still potentially High** for the uninvestigated 1–58 layer. | Investigate `step_1`–`step_58` and `step_82`–`step_86` with the same rigor (read the actual check code, verify by execution whether an equivalent test exists elsewhere, only write a new test for a genuine gap) rather than assuming more mirror tests are needed across the board — the 61-91 investigation shows blindly mirroring every step would mostly duplicate existing coverage. Track progress with a per-step checklist as gaps are confirmed or ruled out. |
...
1. **Mirror `Gravity AI Review Suite.py`'s highest-value `step_*` invariants into pytest** —
   *investigation started, revised finding below; no new mirror tests needed yet.* A systematic,
   read-the-actual-code-and-verify-by-execution investigation of the 20 `step_61`–`step_91` audit
   methods (the later Tier/Task-numbered audits, each a standalone `def step_N(...)` method) found
   **every one already independently covered** by a dedicated, purpose-built `tests/test_*.py` file
   exercising the same real production code path with equal-or-stricter assertions — e.g.
   `step_78_advisory_agent_audit` ↔ `tests/test_advisory_agent.py`, `step_89_rolling_beta_lookahead_audit`
   ↔ `tests/test_indicators_lookahead.py`'s perturbation proofs, `step_81_robinhood_execution_bridge_audit`
   ↔ `tests/test_queue_builder.py` + `tests/test_preflight.py`. This revises the original framing: the
   risk is NOT that these later steps' underlying behaviors are untested — they are, independently and
   well. The residual risk is (a) that parity was previously unverified/undocumented, so silent drift
   between a Gravity check and its "equivalent" test could go unnoticed, and (b) the EARLIER steps
   (`step_1`–`step_58`, the older config.py/Pandera-schema-era inline audit blocks) are a different
   story: `tests/test_gravity_mirrored_invariants.py`'s own docstring documents finding 2 genuine gaps
   specifically in that earlier layer, which predates most of today's per-feature test files. One
   narrow, low-priority gap surfaced in the 61–91 batch: `step_72`'s check 8 asserts
   `signals.aggregator.CANONICAL_REGIMES == {"RISK ON","NEUTRAL","RECESSION","CREDIT EVENT"}`
   cross-checked against `dto_models.MacroEconomicDTO`'s actual regime strings; no existing test does
   that specific cross-model comparison (existing tests derive fixtures from `CANONICAL_REGIMES`
   itself, which is tautological) — low blast-radius since many other tests hardcode the same regime
   strings elsewhere, but a clean, cheap mirror candidate if this list is revisited.
   **Next step:** extend the same investigation to `step_1`–`step_58` and `step_82`–`step_86` (not yet
   investigated), which is where a genuine gap is actually more likely to be found per the above.
