...
| `webapp/` (Pilots PWA — React/TypeScript, ~separate stack from the Python suite above) | n/a (JS/TS) | **Done — 7 test files / 71 tests, up from 1 file (`mock.test.ts`, mock-API-layer only).** This work happened twice, concurrently and independently: this branch added `@testing-library/react`, a `test-setup.ts`, and tests for `format.ts` + `FollowModal.tsx` (the two items this doc's original recommendation named specifically) — and, in parallel on `main` (a different session, PRs #266–#268, landed and rebased in during this same pass), a more complete effort shipped the identical infrastructure PLUS component tests for `Marketplace`, `PilotDetail`, and `Portfolio` (the three screens this doc's recommendation left as future work) and `client.ts`'s live-client path (mocked `fetch`) — closing the exact "still untested, lower priority" gap this row used to list. On reconciling (this branch rebased onto `main`), `main`'s `FollowModal.test.tsx`/`test-setup.ts`/`vite.config.ts`/`package.json` were kept as the more complete, already live-verified version (PR #268 ran the real FastAPI backend + browser E2E against it); this branch's unique, additive contribution is `format.test.ts` (28 tests) — the one item neither effort's `FollowModal` coverage touched. A CI gate (`.github/workflows/ci.yml`'s `webapp` job — typecheck + `npm test` + build) also landed on `main` during this window and now covers all of the above on every push/PR. | Medium — this is a real, shippable consumer-facing surface; the highest-risk piece (the Follow flow's safety copy and gating behavior) now has thorough, live-verified regression protection, and the remaining screens are covered too. | Closed. `Onboarding.tsx` is the only screen with no component test remaining — low priority (a 3-step wizard with no financial/safety logic). |
...
7. ~~`webapp/` (Pilots PWA) component tests~~ **done — closed twice, independently, and reconciled.**
   This branch added `@testing-library/react` (previously absent; component tests were structurally
   impossible before this) and tests for `format.ts` + `FollowModal.tsx` — the two items this
   recommendation named specifically. Concurrently on `main` (a different session, PRs #266–#268), a
   more complete effort shipped the same infrastructure plus `Marketplace`/`PilotDetail`/`Portfolio`
   screen tests and `client.ts`'s live-client coverage, live-verified against a real running backend.
   Reconciling this branch onto `main` (rebase) kept `main`'s more complete `FollowModal`/setup/config
   files and added this branch's unique `format.test.ts` on top — see the module-level table for the
   full reconciliation detail. Net: 71 tests across 7 files, only `Onboarding.tsx` (no financial/safety
   logic) left without a component test.
