...
| Third-party autonomous coding agent (Jules) | **Production-ready, opt-in** (`JULES_ENABLED` default `False`) | `docs/JULES_INTEGRATION.md`, `data/jules_client.py`, `settings.py::JULES_ENABLED` |
...
5. **Model alignment**
   - **Partially implemented**. Alignment is governed mostly by skill-level prose (`.claude/skills/robinhood-execution/SKILL.md`, `.claude/skills/jules-delegation/SKILL.md`), plus one structural check on prompt *content* itself: `prompt_registry/guardrails.py::validate_prompt()` deny-lists phrases instructing a model to disable a platform safety gate and requires every `master_preprompt` version to name `ADVISORY_ONLY`. There is no custom model fine-tuning anywhere in this platform.
...
- **No per-agent identity/least-privilege model**: the platform token-gates by API surface (`api/auth.py`), using single shared secrets (`FOLLOW_API_TOKEN`, `ORCHESTRATOR_DAEMON_TOKEN`) with no per-caller subject claim — leaving no distinction in identity or permission tiers among the different agents.
- **Prose-only confirmation gates**: the Robinhood execution per-order confirmation and the Jules `confirm=True` gate are enforced solely by LLM prompt instructions and skill prose (backstopped only by a pinned-invariant test and a dispatch ledger, respectively), lacking robust code-level runtime guarantees comparable to the Alpaca/FMP live-execution path's server-side `pending_approval` state.
