...
| Decision journal | **Production-ready** — lives in `shared/` (not the decommissioned `legacy/streamlit_command_center/`) because it has no Streamlit dependency and is the live backing store read by `api/pilots_api.py`/`pilots/calibration.py` and surfaced in `webapp/src/screens/Calibration.tsx` | `shared/decision_log.py` |
