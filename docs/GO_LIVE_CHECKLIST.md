...
- [ ] `FRED_API_KEY` rotated within the last **90 days**.  
  Set `FRED_KEY_ROTATED_DATE=YYYY-MM-DD` in `.env` to enable automated check. (check wired in Stage 3 of the 2026-06-26 cleanup plan)
- [ ] `ALPACA_API_KEY` + `ALPACA_SECRET_KEY` rotated within the last **90 days**.  
  Set `ALPACA_KEY_ROTATED_DATE=YYYY-MM-DD` in `.env` to enable automated check. (check wired in Stage 3 of the 2026-06-26 cleanup plan)
