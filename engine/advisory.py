...
def evaluate(
    ...
    *,
    ...
    # Tracks whether garch_vol was fit against THIS call's bars_df (True) vs.
    # supplied by the caller via precomputed_garch, which may have been fit
    # against a different bars window fetched at a different point in the
    # cycle (False). Only a same-window fit is safe to also reuse as Step 6's
    # forecast sigma below -- a mismatched-window value is fine for sizing
    # (its pre-existing use) but must not silently become this specific
    # bars_df/current_price forecast's confidence band too.
    garch_vol_fit_on_local_bars = False
    if precomputed_garch is not None and precomputed_garch > 0:
        ...
        try:
            ...
            garch_vol_fit_on_local_bars = True
        except Exception as exc:
    ...
    if precomputed_forecast is not None and precomputed_forecast > 0:
        ...
        try:
            ...
            fc_results = fe.generate_forecast(
                ...
                history_df=bars_df,
                precomputed_garch_annual_vol=(
                    garch_vol if garch_vol_fit_on_local_bars else None
                ),
            )
