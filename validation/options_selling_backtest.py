...
def simulate_vrp_iron_condor_returns(
    ...
    *,
    ...
    """Real, daily strategy-return series for the VRP Iron Condor proxy
    strategy over ``[start, end]`` (inclusive). See the module docstring's
    HONESTY CONTRACT for the full list of documented simplifying
    assumptions.

    Serves two callers with one implementation:
      * ``STRATEGY_REGISTRY["vrp_premium_selling"]``'s adapter, which passes
        its own already-downloaded ``closes`` for the requested validation
        window.
      * ``validation.stress_scenarios.run_stress_tests``'s
        ``stress_returns_fn`` (``ReturnsFn = Callable[[str, str], pd.Series]``
        — called with ONLY ``start``/``end``, `closes=None`), where this
        function downloads its own SPY history (padded with real lookback)
        since a dated shock window like OCT_2008 falls outside whatever
        range the CLI itself requested.

    Returns
    -------
    pd.Series
        Daily returns (fraction, gross of transaction cost), indexed by
        real trading date. Never raises: any unrecoverable data gap
        degrades to an all-zero (flat/cash) series for the affected span,
        never a fabricated nonzero return (CONSTRAINT #4). An empty
        ``closes``/download degrades to an empty Series.
    """
    ...
    while pos < n and ohlcv.index[pos] <= requested_end:
        if pos < WARMUP_TRADING_DAYS:
            # Not enough real trailing history yet for a causal GARCH fit /
            # 252-day IVR-rank window -- flat/cash, never a fabricated
            # score (CONSTRAINT #4).
        ...
        # Strictly up to and including `pos` -- no lookahead.
        ...
        gate_open = (
            not pd.isna(garch_vol) and not pd.isna(ivr_proxy) and not pd.isna(vrp_proxy)
            and ivr_proxy > IVR_SELL_THRESHOLD and vrp_proxy > VRP_MIN_THRESHOLD
            and macro_dto is not None and macro_dto.vix < VIX_MAX_THRESHOLD
            and macro_dto.market_regime != "CREDIT EVENT"
        )
        ...
        directive = recommender.generate_strategy_pricing_matrix(
            true_ivr=float(ivr_proxy), current_iv=float(garch_vol), trend_bias="Neutral",
            target_dte=TARGET_DTE, vrp=float(vrp_proxy), macro_dto=macro_dto,
        )
        legs = directive.get("Legs", [])
        net_credit = float(directive.get("Net_Premium", 0.0))
        if directive.get("Strategy") != "Iron Condor" or len(legs) != 4 or net_credit <= 0.0:
            # Gate said "sell" but the real pricing engine declined to
            # construct a well-formed Iron Condor (e.g. degenerate strikes)
            # -- flat, never a fabricated fill.
            for d in cycle_dates:
                daily_returns[d] = 0.0
            pos = cycle_end_pos
            continue
        ...
        # Real max loss of an Iron Condor: the wider wing's strike-width
        # minus the credit received -- the honest denominator for turning
        # dollar P&L into a return.
        put_width = abs(k_short_put - k_long_put)
        call_width = abs(k_long_call - k_short_call)
        max_risk = max(put_width, call_width) * 100.0 - net_credit * 100.0
        if max_risk <= 0.0:
            for d in cycle_dates:
                daily_returns[d] = 0.0
            pos = cycle_end_pos
        ...
        sigma = float(garch_vol)  # constant entry-sigma through the cycle (documented assumption)
        # `cumulative_pnl` is the running mark-to-market P&L since entry:
        # net_credit received minus what it would cost to close right now.
        # At entry this is ~0 (a fairly-priced position has no free lunch
        # the instant it's opened) -- NOT `net_credit * 100`, which would
        # double-count the credit as both "cash already banked" and "value
        # still owned", fabricating a huge phantom day-0 loss when
        # `cost_to_close` (correctly, ~net_credit) is first subtracted.
        cumulative_pnl = 0.0
        ...
        for i, d in enumerate(cycle_dates):
            ...
            # Short the condor: liability is the cost to close (buy back
            # the shorts, sell the longs).
            cost_to_close = (mtm_short_put - mtm_long_put) + (mtm_short_call - mtm_long_call)
            new_cumulative_pnl = (net_credit - cost_to_close) * 100.0
            daily_pnl = new_cumulative_pnl - cumulative_pnl
            daily_returns[d] = daily_pnl / max_risk
            cumulative_pnl = new_cumulative_pnl

            if -cumulative_pnl > STOP_LOSS_CREDIT_MULTIPLE * net_credit * 100.0:
                stop_triggered = True
