...
- **scripts/preflight_check.py** — Programmatic pre-live readiness gate. Each of 15 checks returns a `CheckResult(name, passed, reason, warning)`. Exits code **0** only when ALL checks pass. CLI: `python scripts/preflight_check.py [--json] [--skip check1 check2 ...]`. Checks (in order): `fred_key_configured`, `key_rotation_recent` (FRED_KEY_ROTATED_DATE; warning-only), `advisory_only_active` (Tier 5.1 quarantine status; warning-only when False), `alpaca_configured`, **`macro_regime_gate_enabled`** (blocking failure when `MACRO_REGIME_GATE_ENABLED=false` AND `ALPACA_PAPER=false`; warning-only in paper mode), `alpaca_paper_mode` (warning-only, not blocking), `dry_run_disabled`, `env_not_committed`, `kill_switch_inactive`, **`state_snapshot_fresh`** (< 2 h — cross-mode liveness indicator; both `main.py` and `main_orchestrator.py` write `output/state_snapshot.json`; NOT auto-skipped in advisory mode because it IS the advisory liveness check), `heartbeat_fresh` (< 2 h — orchestrator-only; auto-skipped in advisory mode), `db_exists`, `paper_trading_duration` (≥ 90 days — requires `PAPER_TRADING_START_DATE` in `.env`), `validation_reports` (all deployable + < 30 days old), `no_unexpected_risk_blocks` (no `minimum_validation` blocks in last 24 h). All checks catch exceptions rather than propagating them. `_ADVISORY_AUTO_SKIP` (7 entries): `alpaca_configured`, `alpaca_paper_mode`, `dry_run_disabled`, `paper_trading_duration` (broker-dependent) + `heartbeat_fresh`, `validation_reports`, `no_unexpected_risk_blocks` (advisory false-positives — these fail in advisory mode even on a healthy platform). **Stage 2 (2026-06):** added `check_state_snapshot_fresh` as check #10, expanded `_ADVISORY_AUTO_SKIP` from 4 to 7 entries, updated `FRED_KEY_ROTATED_DATE` in `.env.example`, replaced all `datetime.utcnow()` deprecation warnings with `datetime.now(timezone.utc)` in production and test code. Gravity step_66 audits the Alpaca rotation check.
...
- **`scripts/preflight_check.py`** —
  ...
  - Module-level constant `_ADVISORY_AUTO_SKIP` (7 entries): broker-dependent checks (`alpaca_configured`, `alpaca_paper_mode`, `dry_run_disabled`, `paper_trading_duration`) + advisory false-positive checks (`heartbeat_fresh`, `validation_reports`, `no_unexpected_risk_blocks`). **Stage 2 expanded from 4 → 7 entries** to eliminate false-positive failures on a correctly-running advisory deployment: `heartbeat_fresh` fails because only `main_orchestrator.py` writes `heartbeat.txt` (not `main.py`); `validation_reports` gates live deployment, not advisory operation; `no_unexpected_risk_blocks` requires order submissions which never occur in advisory mode. `state_snapshot_fresh` is deliberately NOT in `_ADVISORY_AUTO_SKIP` — it is the advisory liveness indicator (both entry points write `state_snapshot.json`).
...
- **`tests/test_preflight.py`** — extended in Stage 2 with `TestStateSnapshotFresh` (fresh/stale/missing/mtime-fallback/not-in-auto-skip) and `TestAdvisoryAutoSkip` (all 7 entries present, state_snapshot_fresh excluded, run_checks applies auto-skip). Total: 41 tests.

### Gravity step 54
- **`step_54_advisory_only_audit`** — 9 checks (updated in Stage 2, check 6): default ADVISORY_ONLY=True; `main_orchestrator.py` source references ADVISORY_ONLY + the quarantine log message; `gui/panels.py` source has the ADVISORY_ONLY guard + "Advisory mode — broker execution disabled" banner string; `gui/app.py` source renders the "ADVISORY MODE" banner; `preflight_check.check_advisory_only_active` exists; **`_ADVISORY_AUTO_SKIP` contains all 7 advisory-mode auto-skip entries** (4 broker + 3 false-positives; check 6 now uses superset test not exact equality); functional skip path under `ADVISORY_ONLY=True`; `check_advisory_only_active` warns under `ADVISORY_ONLY=False`; `tests/test_advisory_only.py` exists.

### Gravity step 66 (Stage 2)
- **`step_66_advisory_false_positive_audit`** — 10 checks: `check_state_snapshot_fresh` exists + in `ALL_CHECKS`; `_ADVISORY_AUTO_SKIP` contains all 7 expected entries; `state_snapshot_fresh` NOT in `_ADVISORY_AUTO_SKIP`; fresh snapshot → PASS + missing snapshot → FAIL (fail-closed); stale snapshot (>2h) fails via timestamp field; `heartbeat_fresh` auto-skipped; `validation_reports` + `no_unexpected_risk_blocks` auto-skipped; `ALL_CHECKS` count == 15; `tests/test_preflight.py` contains both new test classes.
