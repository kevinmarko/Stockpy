...
### Surface
- `llm/schemas.py::ResearchBrief` — qualitative-only, bounded fields (`thesis_context`≤600, `catalysts` 1-4×≤160, `risk_factors` 1-4×≤160, `recent_developments` 0-4×≤200, `data_confidence` Literal[low|medium|high], `sources_note`≤200). Structurally NO numeric field — every field resolves to `str`/`list[str]`/`Literal[...]`, so there is nothing to fabricate a price target or score into (stronger than a field-name scan; see Gravity step_77 check 4).
...
- `llm/router.py::get_research_provider()` — gated on `OPAL_RESEARCH_ENABLED` + `OPAL_RESEARCH_PROVIDER=="openai"` (the only supported provider today — Opal is NOT flexibly routed like the Claude/Gemini rationale/alert jobs) + `OPENAI_API_KEY`.
- `engine/advisory.py` — `Recommendation.research_brief: Optional[Dict[str,Any]] = None` (additive trailing field; existing positional `Recommendation(...)` constructions elsewhere are unaffected). `enrich_with_llm_rationale()` now runs Opal FIRST (front-of-pipeline): builds a local `working_context` copy (never mutates the caller's `context` dict), generates the brief when `OPAL_RESEARCH_ENABLED`, injects it into `working_context["research_brief"]`, THEN calls `generate_analyst_rationale(rec_skeleton, working_context)` so Claude's own synthesis can cite it. Opal's failure and Claude's failure are each independently soft-fail — one succeeding while the other fails still produces a `replace()`'d `Recommendation` with just that one field populated. No top-level `openai` import.
- `llm/commentary.py::_format_rationale_user_prompt(rec_skeleton, context=None)` — appends a "Research context" block (thesis_context/catalysts/risk_factors/recent_developments/data_confidence/sources_note) when `context["research_brief"]` is present; purely additive — absent/empty `context` produces byte-identical output to pre-Opal behavior.
...
- **`CAPABILITIES`** — registry of all five options in display order: `claude_commentary` (label: "Analyst rationale commentary"), `gemini_alerts` (label: "Alert commentary"), `gemini_vision`, `gravity_ai_runner`, `opal_research`. The two Tier-9 commentary rows are flexibly routed — `claude_commentary` sets `provider_selector_setting="LLM_COMMENTARY_RATIONALE_PROVIDER"`, `gemini_alerts` sets `provider_selector_setting="LLM_COMMENTARY_ALERT_PROVIDER"` (their historical key names are kept for backward compatibility with existing tests/Gravity checks even though either job may now run on either provider). `gemini_vision` / `gravity_ai_runner` / `opal_research` are NOT flexibly routed (`provider_selector_setting=None`) — each has a fixed provider.
