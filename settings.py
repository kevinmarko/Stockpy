...
class Settings(BaseSettings):
    ...
    FORECAST_SKILL_WEIGHTING_ENABLED: bool = Field(
        ...
        description=(
            ...
            "IMPORTANT (2026-09 fix, see docs/known_issues/"
            "forecast_available_coupled_to_skill_weighting_flag.md): this flag gates "
            "ONLY the skill-weighted BLENDING read-back "
            "(ForecastTracker.get_skill_weights), NOT whether a ForecastTracker is "
            "attached at all. A ForecastTracker is now always threaded into every "
            "ForecastingEngine construction (main_orchestrator.py, engine/advisory.py, "
            "pipeline/production_steps.py) regardless of this flag, so "
            "record_forecasts/update_actuals — and therefore "
            "ForecastTracker.get_covered_symbols()/forecast_available telemetry "
            "surfaced by GET /data/sync-report — reflect real per-cycle forecast "
            "coverage on every deployment, not only ones with this flag on. Before "
            "this fix, the one flag also silently gated recording itself, so "
            "forecast_errors stayed empty and forecast_available reported False for "
            "every symbol whenever this flag was left at its default. When True, a "
            "persistent ForecastTracker is threaded into every ForecastingEngine "
            "construction, self-provisioning its forecast_errors table in "
            "quant_platform.db (no migration required)."
        ),
    )
