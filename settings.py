...
class Settings(BaseSettings):
    ...
    CNN_LSTM_SUBPROCESS_ISOLATION_ENABLED: bool = Field(
        ...
        description=(
            ...
            "longer matter, unlike the module-level reorder alone or the entry-point "
            "guards. This is what actually removes the process-scope constraint, "
            "rather than merely mitigating it by convention: it protects EVERY "
            "caller, known or not, not just the three files that remember the guard. "
            "All feature engineering / windowing / scaling stays in the parent "
            "process unchanged (pandas-only, never touches TF). Any subprocess "
            "failure (timeout, BrokenProcessPool, real training exception) is caught "
            "by run_cnn_lstm_forecast's existing outer try/except and degrades to "
            "the zero-result sentinel -- never crashes the pipeline (CONSTRAINT #6). "
            "This default flipped True on 2026-07-31 (Round 7 of the known-issues "
            "doc) once Round 6 (2026-07-27) verified subprocess isolation end-to-end "
            "against the real native deadlock on real production data in the actual "
            "macOS arm64 + Framework-Python environment the deadlock was originally "
            "confirmed on -- the earlier caveat about this being verified only "
            "against the mocked test suite no longer applies. Set False only to "
            "restore the legacy in-process path (byte-identical to this flag's "
            "original pre-2026-07-31 default); doing so re-exposes the process-scope "
