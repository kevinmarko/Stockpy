...
def fetch_and_cache_universe() -> pd.DataFrame:
    ...
    try:
        ...
        # Avoid literal html warning using io.StringIO
        tables = pd.read_html(io.StringIO(resp.text))

        resp_changes = requests.get(changes_url, headers=headers, timeout=15)
        resp_changes.raise_for_status()
        changes_tables = pd.read_html(io.StringIO(resp_changes.text))
    ...
    if not changes_tables:
        raise ValueError("Wikipedia changes page structure changed. S&P 500 changes tables not found.")
