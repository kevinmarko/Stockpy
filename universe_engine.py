...
def fetch_and_cache_universe() -> pd.DataFrame:
    ...
    try:
        ...
        import io
        tables_current = pd.read_html(io.StringIO(resp_current.text))

        resp_changes = requests.get(url_changes, headers=headers, timeout=15)
        resp_changes.raise_for_status()
        tables_changes = pd.read_html(io.StringIO(resp_changes.text))
    ...
    if not tables_current:
        raise ValueError("Wikipedia page structure changed. Current S&P 500 table not found.")
    if not tables_changes:
        raise ValueError("Wikipedia page structure changed. Historical S&P 500 table not found.")
