...
def render_help() -> None:
    ...
    from shared.onboarding import read_onboarding_state, mark_onboarded, DEFAULT_MARKER
    from shared.help_content import GLOSSARY, search_glossary
    ...
    if _ob_state.should_show:
        ...
        with st.expander("✅ Start here — 4-step checklist", expanded=True):
            ...
            if st.button("✅ Got it — don't show again"):
                ...
                st.session_state[__import__("shared.onboarding", fromlist=["SESSION_KEY"]).SESSION_KEY] = True
                st.rerun()
        st.divider()
