...
def _render_launcher_safety_controls() -> None:
    ...
    *   **🔵 DRY RUN** checkbox → writes ``DRY_RUN`` to ``.env`` via
        :func:`shared.env_io.write_setting` (allowlist-bounded).
...
def _render_maintenance_diagnostics() -> None:
    ...
    *   **📝 Generate today's briefing** — ``run_daily_briefing()`` (blocking,
        subprocess-captured via ``shared.command_runner``). Result also lands in
    ...
    *   **✅ Run tests (pytest)** / **🧪 Run full verify (make verify)** — both
    ...
    Every import of the parallel-built runner modules (``shared.command_runner``
    for the blocking commands, ``launch_pytest``/``launch_verify`` on
    ``shared.orchestrator_runner`` for the long-running ones) is done *inside* the
    ...
    with st.expander("🔧 Maintenance & Diagnostics", expanded=False):
        ...
        with c_brief:
            if st.button(
                ...
                try:
                    from shared.command_runner import run_daily_briefing
                except ImportError:
        ...
        with c_db:
            if st.button(
                ...
                try:
                    from shared.command_runner import run_database_setup
                except ImportError:
                    st.info("Command runner not available in this build.")
                else:
                    with st.status(
        ...
        with c_pytest:
            if st.button(
                ...
                try:
                    from shared.orchestrator_runner import launch_pytest
                except ImportError:
        ...
        with c_verify:
            if st.button(
                ...
                try:
                    from shared.orchestrator_runner import launch_verify
                except ImportError:
...
def _render_dead_letter_queue() -> None:
    ...
    **🔄 Retry** button that spawns ``main.py`` for just that symbol via
    :func:`shared.orchestrator_runner.launch_symbol_retry`.
    """
    from shared.dead_letter import DEAD_LETTER_PATH, read_dead_letter
...
def _render_robinhood_execution_status() -> None:
    ...
    from shared.robinhood_mode import read_robinhood_execution_mode
    from shared.help_content import SECTION_HELP
    from shared import help_widgets
