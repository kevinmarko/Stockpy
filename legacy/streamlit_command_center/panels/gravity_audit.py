...
# ``legacy.streamlit_command_center.panels.load_state_snapshot``). Behaviour-preserving: WHAT renders is
...
def _load_gravity_report_cached(path_str: str, _mtime: float) -> List[Dict[str, Any]]:
    ...
    ``strategies`` list (delegates to :func:`shared.strategy_health.read_gravity_report`)."""
...
def _load_gravity_report() -> List[Dict[str, Any]]:
    ...
    from shared.strategy_health import _DEFAULT_REPORT_PATH
...
def _render_circuit_breaker_dashboard() -> None:
    ...
    Read-only derivation via :mod:`shared.circuit_breakers`. Adding a new
...
def _render_strategy_health() -> None:
    ...
    """
    from shared.strategy_health import DeployabilityGate, evaluate_gate
    ...
    for strategy_dict in strategies:
        ...
        with st.expander(
            ...
            if has_sharpe_row:
                ...
                try:
                    from shared.styling import _color_sharpe
...
def _render_gravity_ai_runner_section() -> None:
    ...
    Four render paths driven by :func:`shared.gravity_ai_panel.runner_status`:
...
def render_gravity_audit() -> None:
    ...
    2.  **Circuit Breaker Dashboard** — every tripped breaker derived from the
        ...
        :mod:`shared.circuit_breakers`.
    3.  **Dependency Map** — declarative source → consumer graph from
        :mod:`shared.dependency_map`. The operator picks the degraded sources
