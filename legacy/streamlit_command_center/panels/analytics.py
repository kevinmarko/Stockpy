...
signals-analytics panels (``legacy.streamlit_command_center.panels.analytics_signals``); that import + the
...
from shared.help_content import MODEL_RETRAIN_WINDOW_DAYS, metric_help
...
# Sourced from shared.help_content.MODEL_RETRAIN_WINDOW_DAYS (which itself mirrors
...
# ``legacy.streamlit_command_center.panels.load_state_snapshot``). Behaviour-preserving: WHAT is shown is
...
def _render_rolling_beta_chart(snap: Dict[str, Any]) -> None:
    ...
    """
    from legacy.streamlit_command_center.panels._shared import _active_symbols
...
def render_analytics() -> None:
    ...
    sibling ``legacy.streamlit_command_center.panels.analytics_signals`` panels (ML registry / news
    ...
    try:
        from legacy.streamlit_command_center.panels import analytics_signals
