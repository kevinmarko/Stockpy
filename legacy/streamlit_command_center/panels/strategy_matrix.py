"""InvestYo Command Center — Strategy Matrix & Risk Gating tab. Enumerates the registered signal modules, edits their weights, toggles modules on/off (DISABLED_SIGNAL_MODULES), and controls the global kill switch, all via the allowlist-bounded shared.env_io writer."""
...
# Imported lazily-safe (legacy.streamlit_command_center.panels.__init__ imports this module after defining
...
def _render_strategy_mode_toggle() -> None:
    ...
    Backed by :func:`shared.strategy_registry.set_active_mode`, which writes
    ``DRY_RUN`` and ``ALPACA_PAPER`` to ``.env`` via the allowlist-bounded
    :mod:`shared.env_io` writer. Effect on the **next** orchestrator launch — we
...
def _render_strategy_version_registry() -> None:
    ...
    Backed by :func:`shared.strategy_registry.list_strategy_versions`. Useful for
    answering "did I really redeploy the meta-labeler since last week's run?"
    without having to scroll git log.
    """
    from shared.strategy_registry import list_strategy_versions
...
def render_strategy_matrix() -> None:
    ...
    1.  **Global Execution Mode** — Simulation / Paper / Live selector backed
        by :mod:`shared.strategy_registry`. Writes ``DRY_RUN`` + ``ALPACA_PAPER``
        to ``.env`` via the allowlist-bounded :mod:`shared.env_io` writer.
