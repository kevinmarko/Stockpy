"""
legacy/streamlit_command_center/panels/_shared.py
====================================================
Shared constants and utility helpers used by every panel module.  Extracted
from ``legacy/streamlit_command_center/panels/__init__.py`` (was
``gui/panels/__init__.py``) so individual tab modules can import exactly
...
remain in ``legacy/streamlit_command_center/panels/__init__.py`` so the test
suite can monkeypatch them on the ``legacy.streamlit_command_center.panels``
...
+ ``shared.report_viewer_helpers`` (for ``GICS_SECTORS``/``_BF_EDITOR_COLUMNS``,
which live there natively — see that module's own comment for why).
No imports from other ``legacy.streamlit_command_center.panels.*`` sub-modules — this file is the base of
the dependency tree.
"""
...
# GICS_SECTORS / _BF_EDITOR_COLUMNS now live natively in
# shared/report_viewer_helpers.py (moved there so that module — which lives
# outside the archived Streamlit UI — no longer needs a lazy circular import
# back into this package). Re-exported here so every panel's existing
# `from legacy.streamlit_command_center.panels._shared import GICS_SECTORS`
# keeps working unchanged.
from shared.report_viewer_helpers import GICS_SECTORS, _BF_EDITOR_COLUMNS

logger = logging.getLogger("legacy.streamlit_command_center.panels")

# Repo root: legacy/streamlit_command_center/panels/_shared.py → panels/ →
# streamlit_command_center/ → legacy/ → repo/
...
# in gui/panels/__init__.py so tests can monkeypatch them on the legacy.streamlit_command_center.panels
# namespace without chasing module-reference indirection.
# ===========================================================================
...
# ---------------------------------------------------------------------------
# Cross-tab regime filter — session-aware glue over shared.regime_filter.
# ---------------------------------------------------------------------------
...
# pure shared.regime_filter logic so the shared state-snapshot loader can hand
...
def active_regime_filter() -> str:
    ...
    """
    from shared.regime_filter import ALL_REGIMES_LABEL
...
def apply_session_regime_filter(snapshot):
    ...
    delegates to :func:`shared.regime_filter.filter_snapshot`. The "All regimes"
    ...
    """
    from shared.regime_filter import filter_snapshot
