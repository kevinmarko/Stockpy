"""InvestYo Command Center — AI Control Center tab. Renders master on/off switches and provider/model selectors for the platform's LLM capabilities, writing the chosen settings through the allowlist-bounded shared.env_io writer and applying them immediately to this process's live settings (see shared.ai_control_center.LIVE_PATCHABLE_KEYS) — a separately-running process (e.g. the orchestrator daemon) still needs its own restart."""
...
from legacy.streamlit_command_center.panels.ai_insights import _render_gemini_chart_section
from legacy.streamlit_command_center.panels.gravity_audit import _render_gravity_ai_runner_section
from legacy.streamlit_command_center.panels.report_viewer import _render_llm_commentary_button
...
# field named on ``shared.ai_control_center.AICapability``). The AICapability
...
# shared.ai_control_center._PROVIDER_KEY_MAP; Opal routes to OpenAI OR Gemini).
...
def _render_capability_row(
    ...
    *,
    ...
    cached (see ``shared.ai_control_center.LIVE_PATCHABLE_KEYS``), so this is
...
def render_ai_control_center() -> None:
    ...
    * **A — Capability grid + toggles.** One row per AI option (Claude
      ...
      enable/disable toggle written via ``shared.env_io`` and applied
    ...
    * **D — Operator-launched scheduled run.** Start/Stop buttons that spawn /
      ...
      ``shared.orchestrator_runner``. You start it, you stop it — nothing runs on
      its own.
    """
    ...
    # reads the store; shared.ai_control_center itself never does, staying
    # filesystem-free.
    try:
    ...
    if not sig_list:
        ...
        if symbols:
            ...
            try:
                from shared.ai_control_center import opal_built  # noqa: PLC0415
