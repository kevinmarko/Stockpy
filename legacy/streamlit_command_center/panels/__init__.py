...
``from legacy.streamlit_command_center.panels import render_launcher`` unchanged.
...
"legacy.streamlit_command_center.panels -> legacy.streamlit_command_center.panels.ai_control_center -> ... -> legacy.streamlit_command_center.panels" because
every ``from legacy.streamlit_command_center.panels.<submodule> import X`` here creates an implicit
...
# backward compatibility so all existing ``from legacy.streamlit_command_center.panels import X`` imports
...
# ``legacy.streamlit_command_center.panels._load_state_snapshot_cached`` on the panels namespace without
...
from .sentiment_dynamics import render_sentiment_dynamics
