"""InvestYo Command Center — Prompt Registry tab. Version control for AI-facing prompts: shows each prompt ID's resolved version + source, a Sync button, a per-ID diff viewer, and a rollback action that pins the chosen version via shared.env_io (effective on next launch)."""
...
def render_prompt_registry() -> None:
    ...
    :mod:`shared.env_io` writer (effective on the **next** orchestrator launch — the
    ...
    **Security banner** (always rendered):
    ...
    - CONSTRAINT #3: secrets are never displayed; pins are written via
      ``shared.env_io.write_setting`` (``PROMPT_REGISTRY_PINS`` is in ``ALLOWED_KEYS``).
    ...
    with st.expander("↩ Rollback / pin", expanded=False):
        st.caption(
            ...
            "Written to `.env` via the allowlist-bounded `shared.env_io` writer."
        )
