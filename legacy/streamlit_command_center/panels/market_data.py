...
# ``legacy.streamlit_command_center.panels.load_state_snapshot``). Behaviour-preserving: the returned symbol
...
def render_market_data() -> None:
    ...
    *   **Connectivity badge** — sliding-window success rate from
        :class:`shared.market_data_diagnostics.FetchHealthTracker` (Healthy /
        Degraded / Down), persisted across reruns in ``st.session_state``.
    *   **Throttled batch fetch** — uses
        :class:`shared.market_data_diagnostics.BatchQuoteFetcher` with default
    ...
    *   **Quote validation** — :func:`validate_quote` flags NaN price, missing
    ...
    from shared.observability_telemetry import LatencySampleStore
