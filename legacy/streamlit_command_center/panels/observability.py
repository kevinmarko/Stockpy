...
def render_observability() -> None:
    ...
    2.  Macro Regime Gate         — operator toggle (MACRO_REGIME_GATE_ENABLED)
                                    ...
                                    Writes the setting to .env via shared.env_io
                                    (CONSTRAINT #3).
...
# ``shared.styling`` Styler helpers (``_color_pnl`` / ``style_severity``) instead
...
def _load_account_snapshot_cache() -> Dict[str, Any]:
    ...
    the same file :func:`legacy.streamlit_command_center.panels._shared._held_symbols` reads, but this
...
def _render_observability_account_holdings() -> None:
    ...
    via :func:`shared.styling.style_severity`.
    """
...
# :func:`shared.observability_panel_helpers.compute_portfolio_heat` returns NaN and
...
def _load_validation_reports() -> List[Dict[str, Any]]:
    ...
    Same glob :func:`legacy.streamlit_command_center.panels.gravity_audit._render_validation_stress_regime_section`
...
def _forecast_skill_rows(
    ...
    * ONE :class:`ForecastTracker` (its methods reuse a single connection after
    ...
    mirroring ``legacy.streamlit_command_center.panels.load_state_snapshot``'s mtime-keying. ``symbols`` is a
...
# Reuses the already-shared ``legacy.streamlit_command_center.panels._shared.load_block_log`` loader
...
def _render_observability_heartbeat_trend() -> None:
    ...
    Implementation
    --------------
    :class:`shared.observability_telemetry.HeartbeatTrendStore` is a 60-sample ring
...
def _render_observability_system_telemetry() -> None:
    ...
    Backed by :func:`shared.observability_telemetry.collect_system_telemetry`,
    which falls back to NaN-shaped output when ``psutil`` is unavailable
    (CONSTRAINT #4 — no fabricated zeros).
    """
    from shared.observability_telemetry import collect_system_telemetry, format_bytes
...
def _render_observability_latency_heatmap() -> None:
    ...
    :class:`shared.observability_telemetry.LatencySampleStore` populated each time
    the operator clicks **Fetch quotes** on the Market Data tab.
    """
    from shared.observability_telemetry import LatencySampleStore, summarise_latency
...
def _render_observability_error_log() -> None:
    ...
    :func:`shared.observability_telemetry.read_log_tail`.
    ...
    * **symbol-specific** — per-ticker failures extracted by :func:`shared.observability_telemetry.extract_symbol_from_message`.
