...
# Resolve repo root so `streamlit run legacy/streamlit_command_center/app.py`
# works from any CWD and the platform's top-level modules (settings, signals,
# execution, …) import cleanly.
...
from shared.export_utils import dataframe_to_csv_bytes, signals_snapshot_to_dataframe
from shared.help_content import metric_help
from shared.regime_filter import ALL_REGIMES_LABEL, apply_regime_filter

logging.basicConfig(level=getattr(logging, str(settings.LOG_LEVEL).upper(), logging.INFO))
logger = logging.getLogger("legacy.streamlit_command_center.app")
...
# at the SHARED snapshot-loading layer: legacy.streamlit_command_center.panels.load_state_snapshot() applies
# it (via shared.regime_filter) so every panel reading the shared snapshot gets a
