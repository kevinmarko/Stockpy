"""
legacy/streamlit_command_center/desktop_shell/ui_server.py
============================================================
Supervisor for the Streamlit Command Center UI
(``legacy/streamlit_command_center/app.py``), launched as a background
subprocess so a native shell (WS4's ``app_shell.py``, built on ``pywebview``)
can host it in a real window instead of a browser tab.

Why a subprocess (mirrors ``shared/orchestrator_runner.py``)
--------------------------------------------------------------
...
``shared/orchestrator_runner.launch_orchestrator`` for ``main_orchestrator.py``
...
# Nesting-depth-independent (this file has moved twice already across
# refactors) — anchor off settings.py's own repo-root constant rather than
# re-counting Path(__file__) parents.
_REPO_ROOT = ENV_PATH.parent

# Streamlit UI server launch log — analogous to shared/orchestrator_runner.py's
...
def start_ui_server(port: int, *, headless: bool = True) -> subprocess.Popen:
    ...
    Notes
    -----
    Mirrors the launch pattern in ``shared/orchestrator_runner.py``'s
...
def stop_ui_server(popen: subprocess.Popen, *, timeout: float = 5.0) -> bool:
    ...
    exited). Never raises — mirrors ``shared/orchestrator_runner.py``'s
