...
================================
Thin delegation wrapper over ``shared.orchestrator_runner``'s already-mature,
...
``shared.orchestrator_runner`` internals directly. This module is that surface:
...
def start_engine(interval_seconds: int = 300, *, refresh_account: bool = False):
    ...
    Delegates to :func:`shared.orchestrator_runner.launch_daemon_engine` (the
    persistent orchestrator daemon) when ``settings.ORCHESTRATOR_DAEMON_ENABLED``
    is True, else to :func:`shared.orchestrator_runner.launch_scheduled_advisory`
    ...
    from shared.orchestrator_runner import launch_scheduled_advisory
...
def stop_engine(handle: Any, *, timeout: float | None = None) -> bool:
    ...
    Delegates to :func:`shared.orchestrator_runner.stop_run`. When ``timeout``
    ...
    * anything else (``"scheduled"``, or a handle missing ``mode`` entirely
    ...
    bool
        ...
        ``stop_run``.
    """
    from shared.orchestrator_runner import stop_run
