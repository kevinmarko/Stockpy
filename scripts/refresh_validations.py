...
def run_validations(
    ...
    for name in strategies:
        ...
        try:
            ...
            # Structural (not name-based) MultiIndex-adapter contract: when
            # X's index is a genuine (Date, Ticker) pd.MultiIndex (currently
            # only _build_sector_quality_rank_adapter), the third tuple
            # element is (strategy_fn, t1) instead of the flat
            # Dict[str, pd.Series] every other adapter returns — see that
            # adapter's own docstring. start_date/end_date must be derived
            # from the MultiIndex's Date level (X.index[0] is a tuple for a
            # MultiIndex, so `.date()` would raise AttributeError).
