...
def _fetch_sneqr_quality_facts(ticker: str) -> pd.DataFrame:
    ...
    *ticker*, sourced directly from SEC EDGAR XBRL company facts.
    ...
    Thin wrapper over ``data.sneqr_quality_facts.fetch_sneqr_quality_facts``
    (extracted 2026-09 so this real, live-EDGAR-verified computation has
    exactly one implementation, shared with ``ml/forecast_backfill.py``'s
    Forecast Backfill meta-labeler bridge -- see that module's docstring for
    the full math/PIT-convention detail, unchanged by this extraction) --
    pinned to this script's own ``SNEQR_BACKTEST_START`` so the ~2010 XBRL-
    reliability cutoff documented at that constant's definition stays this
    call site's actual behavior, byte-identical to before the extraction.
    """
    from data.sneqr_quality_facts import fetch_sneqr_quality_facts

    return fetch_sneqr_quality_facts(ticker, since=SNEQR_BACKTEST_START)
...
if __name__ == "__main__":
    ...
    # gui/panels/validation_lab.py and Gravity AI Review Suite.py; a
    # module-top call would fire the re-exec check on every such
    # import, not just when this file is the actual entry point. See
