...
scripts.refresh_validations, and its sibling _fetch_options_strategy_registry
fetches ``sorted(STANDARD_OPTIONS_STRATEGIES.keys())`` from
validation.options_harness -- both via an isolated subprocess, mirroring
cli_introspect/capture.py's isolation philosophy (refresh_validations
heavy-imports pandas/numpy/the quant engines). Neither must ever raise -- any
...
from scripts.build_command_manifest import _fetch_options_strategy_registry, _fetch_strategy_registry
