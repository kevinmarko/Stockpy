...
# ---------------------------------------------------------------------------
# TestCacheKeyFollowsConfiguredProvider — flexible-routing regression guard.
#
# Before flexible routing, the cache key's `provider=` field was hardcoded
# ("claude" for rationale, "gemini" for alerts) — harmless when only one
# provider was ever possible per job. Now that either provider can serve
# either job, the cache key MUST reflect the LIVE-configured provider so
# switching providers (e.g. rationale: claude -> gemini) doesn't serve a
# stale payload actually generated by the other model.
# ---------------------------------------------------------------------------


class TestCacheKeyFollowsConfiguredProvider:
    def test_rationale_switching_provider_does_not_reuse_cache(self, monkeypatch):
        skeleton = {"symbol": "AAPL", "action": "BUY", "key_indicators": {"score": 75.0}}

        monkeypatch.setattr(commentary_mod.settings, "LLM_COMMENTARY_ENABLED", True, raising=False)
        monkeypatch.setattr(
            commentary_mod.settings, "LLM_COMMENTARY_RATIONALE_PROVIDER", "claude", raising=False
        )
        claude_prov = _FakeProvider(_good_rationale())
        monkeypatch.setattr(commentary_mod, "get_rationale_provider", lambda: claude_prov)
        generate_analyst_rationale(skeleton)
        assert claude_prov.call_count == 1

        # Flip the configured provider to gemini — must NOT hit the claude
        # cache entry; the gemini provider must actually be called.
        monkeypatch.setattr(
            commentary_mod.settings, "LLM_COMMENTARY_RATIONALE_PROVIDER", "gemini", raising=False
        )
        gemini_prov = _FakeProvider(_good_rationale())
        monkeypatch.setattr(commentary_mod, "get_rationale_provider", lambda: gemini_prov)
        generate_analyst_rationale(skeleton)
        assert gemini_prov.call_count == 1

    def test_alert_switching_provider_does_not_reuse_cache(self, monkeypatch):
        skeleton = {"symbol": "AAPL", "kind": "momentum_building"}

        monkeypatch.setattr(commentary_mod.settings, "LLM_COMMENTARY_ENABLED", True, raising=False)
        monkeypatch.setattr(
            commentary_mod.settings, "LLM_COMMENTARY_ALERT_PROVIDER", "gemini", raising=False
        )
        gemini_prov = _FakeProvider(_good_alert())
        monkeypatch.setattr(commentary_mod, "get_alert_provider", lambda: gemini_prov)
        generate_alert_commentary(skeleton)
        assert gemini_prov.call_count == 1

        # Flip the configured provider to claude — must NOT hit the gemini
        # cache entry; the claude provider must actually be called.
        monkeypatch.setattr(
            commentary_mod.settings, "LLM_COMMENTARY_ALERT_PROVIDER", "claude", raising=False
        )
        claude_prov = _FakeProvider(_good_alert())
        monkeypatch.setattr(commentary_mod, "get_alert_provider", lambda: claude_prov)
        generate_alert_commentary(skeleton)
        assert claude_prov.call_count == 1
