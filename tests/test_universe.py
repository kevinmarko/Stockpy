...
# These tests make real, unmocked live Wikipedia network calls and exercise
# the on-disk cache read/write path end-to-end. They share one scratch cache
# location for the whole module (test_cache_is_reused/test_point_in_time_
# no_lookahead depend on state written by test_wikipedia_scrape_and_cache
# earlier in the file) so they never read/write the tracked, repo-shipped
# data/universe_cache.parquet -- mirrors the isolation pattern in
# tests/test_dead_letter_resilience.py's _isolated_universe_paths fixture.


@pytest.fixture(scope="module", autouse=True)
def _isolated_cache_path(tmp_path_factory):
    tmp_dir = tmp_path_factory.mktemp("universe_cache")
    mp = pytest.MonkeyPatch()
    mp.setattr(universe_engine, "CACHE_PATH", str(tmp_dir / "universe_cache.parquet"))
    yield
    mp.undo()
...
def test_wikipedia_scrape_and_cache():
    ...
    if os.path.exists(universe_engine.CACHE_PATH):
        try:
            os.remove(universe_engine.CACHE_PATH)
        except OSError:
            pass
    ...
    assert os.path.exists(universe_engine.CACHE_PATH), "Cache parquet file should be created"
...
def test_cache_is_reused():
    ...
    new_mtime = os.path.getmtime(universe_engine.CACHE_PATH)
