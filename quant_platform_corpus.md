    ...
    def calculate_fundamental_metrics(self, fund_dtos):
        ...
        for ticker, dto in fund_dtos.items():
            ...
            try:
                # Gordon Fair Value
                gordon = 0.0
                if dto.dividend_yield > 0:
                    div_amount = dto.dividend_growth_rate
                    growth = 0.03
                    req_return = self.risk_free_rate + (dto.beta * self.market_risk_premium)
                    if req_return > growth:
                        gordon = (div_amount * (1 + growth)) / (req_return - growth)

                # EXPLANATION: Safe extraction of raw yfinance info fields stored on the DTO.
                info = getattr(dto, 'raw_info', {}) or {}
                price = float(info.get('regularMarketPrice', info.get('previousClose', dto.price or 0.0)))
                debt_to_equity = float(info.get('debtToEquity', 0.0)) / 100.0 if info.get('debtToEquity') else None
                historical_vol = dto.eps_trailing # (Proxy fallback if volatility missing)
                inst_own = info.get('heldPercentInstitutions')
                inst_change = info.get('netPercentInsiderShares') or info.get('netPercentInstitutionsSharesOut')
                if not inst_change or inst_change == 0.0:
                    # EXPLANATION: Fallback to the monthly change in short interest as a proxy for institutional velocity,
                    # since Yahoo Finance has removed direct institutional transaction keys from the info dictionary.
                    ...
                    if shares_out > 0:
                        inst_change = (short_prior - short_curr) / shares_out
                ...
                dph = self.research_engine.calculate_dividend_payback_horizon(price, dto.dividend_yield, dto.dividend_growth_rate)
                lev_distress = self.research_engine.calculate_leverage_distress_factor(dto.sector, debt_to_equity)
                ...
                # Use independent calculations to fix duplication:
                gordon_val = self.calculate_gordon_fair_value(price, dto.dividend_yield, dto.dividend_growth_rate)
...
class StrategyEngine:
    ...
    def evaluate_security(self, 
        ...
        if hasattr(macro, 'killSwitch') and macro.killSwitch:
            score_log.append("-50pts: SYSTEMIC KILLSWITCH TRIGGERED (Sahm Rule or VIX Breach)")
            score -= 50
...
if __name__ == "__main__":
    initialize_database()
# ==============================================================================
# MODULE: AI VERIFICATION AND SIMULATION SUITE
# File: AI_Verification_Prompts.py
# Description: This executable acts as the "Testing Sandbox" and LLM Prompting 
#              Harness for your Quantitative AI Agent. It synthesizes Steps 1-6 
...
SYSTEM_PROMPT = """
You are 'Gravity', an Expert Quantitative Python Auditor and Algorithmic Trading Architect.
Your mandate is to perform rigorous static analysis and logical verification of financial
codebases based on institutional-grade quantitative finance standards.

MASTER RULES FOR YOUR REVIEW:
1. You must enforce strict adherence to vectorized operations (Pandas/NumPy). Iteration via loops is an automatic failure.
2. You must check for Lookahead Bias in all time-series and machine learning models.
3. You must verify that mathematical formulas (e.g., Black-Scholes, RSI, MAE, MFE) are implemented correctly.
4. You must ensure risk management (Position Sizing, Portfolio Heat, Slippage limits) is hardcoded into execution logic.

Output your evaluation strictly in valid JSON format matching the requested schema.
No conversational filler. No markdown outside of the JSON block.
"""
...
STEP_2_PROMPT = StepPromptTemplate(
    step_number=2,
    step_title="Implement Advanced Technical Indicators",
    prompt_text="""
Analyze the provided source code for Step 2. Verify the mathematical implementation of the following indicators:
...
3. SIGNAL LOGIC: Confirm that momentum signals require confirmation (e.g., MACD crossover MUST align with RSI extreme recovery).
...
""",
    criteria=[
        ...
        ValidationCriterion("2.2", "ATR Calculation", ["high", "low", "close", "truerange", "rolling"], True)
    ]
)
...
3. HMM REGIMES: Check for Hidden Markov Model (HMM) implementation designed to classify market states (e.g., Bull Volatile, Bear Stable).
4. STATIONARITY: Verify the presence of an Augmented Dickey-Fuller (ADF) test ensuring time-series data is stationary (using fractional differencing or log returns) before prediction.

Respond in JSON: {"status": "PASSED/FAILED", "score": 0-100, "findings": [], "missing_elements": []}
""",
    criteria=[
        ValidationCriterion("4.1", "Data Scaling & Reshaping", ["MinMaxScaler", "reshape", "fit_transform"], True),
        ValidationCriterion("4.2", "Stationarity Checks", ["adfuller", "diff", "log"], True)
    ]
)
...
2. FUNDAMENTAL VALUATION: Must include logic to screen P/E Ratios, Free Cash Flow Yield, and Dividend Growth Rates.
3. REGIME GOVERNANCE: Verify an override mechanism where if the Yield Curve is inverted AND Credit Spreads are widening, the system forces a "RISK OFF" state, halting long equity executions.

Respond in JSON: {"status": "PASSED/FAILED", "score": 0-100, "findings": [], "missing_elements": []}
""",
    criteria=[
        ValidationCriterion("5.1", "Macro Data Ingestion", ["FRED", "Yield Curve", "Sahm"], True),
        ValidationCriterion("5.2", "Fundamental Override", ["P/E", "RISK OFF", "Dividend"], True)
    ]
)
...
1. EXCURSION METRICS: Ensure formulas exist to track Maximum Favorable Excursion (MFE) and Maximum Adverse Excursion (MAE) for every closed trade.
2. SLIPPAGE & SHORTFALL: Validate the calculation of Implementation Shortfall (Arrival Price vs. Executed Price) and execution slippage modeling.
...
4. PORTFOLIO HEAT: Check that the system calculates total open risk across all positions and halts execution if portfolio heat exceeds a defined threshold (e.g., 6%).

Respond in JSON: {"status": "PASSED/FAILED", "score": 0-100, "findings": [], "missing_elements": []}
""",
    criteria=[
        ValidationCriterion("6.1", "MAE & MFE tracking", ["MAE", "MFE", "Maximum Adverse Excursion"], True),
        ValidationCriterion("6.2", "ATR Position Sizing", ["Account", "Risk", "ATR", "Slippage"], True)
    ]
)
...
class GravityAIAuditor:
    ...
    for required terminology, and interacts with the LLM API to get the final JSON.
    """

    def __init__(self, api_key: str = ""):
        self.api_key = api_key
        # In a production environment, initialize openai.AsyncClient or similar here
        logger.info("Gravity AI Auditor Initialized.")

    def _static_regex_pre_check(self, code_content: str, criteria: List[ValidationCriterion]) -> List[str]:
        """Performs a fast static check before spending LLM tokens."""
        missing = []
        for crit in criteria:
            found = any(re.search(r'\b' + re.escape(kw) + r'\b', code_content, re.IGNORECASE) for kw in crit.required_keywords)
            if not found and crit.critical:
                missing.append(f"Critical keyword group missing: {crit.required_keywords}")
        return missing

    def analyze_step(self, step_template: StepPromptTemplate, target_code: str) -> AIReviewReport:
        """
        Constructs the payload, pre-checks the code, and asks the LLM to verify the step.
        """
        logger.info(f"Analyzing Step {step_template.step_number}: {step_template.step_title}")

        # 1. Perform static pre-check
        missing_criticals = self._static_regex_pre_check(target_code, step_template.criteria)
        if missing_criticals:
            logger.warning(f"Static Pre-Check Failed for Step {step_template.step_number}. Missing keys: {missing_criticals}")
            # We can still pass it to the LLM, but we append the static warnings

        # 2. Construct final LLM payload
        full_prompt = f"""
{SYSTEM_PROMPT}
...
"""
        ...
        # 3. Simulate LLM Call (Replace with openai.chat.completions.create in production)
        response_json = self._mock_llm_api_call(full_prompt, len(missing_criticals) == 0)

        # 4. Parse the output into our standardized DataClass
        try:
            parsed = json.loads(response_json)
            # Combine static findings with LLM findings
            all_missing = missing_criticals + parsed.get("missing_elements", [])

            report = AIReviewReport(
                step_number=step_template.step_number,
                status="PASSED" if parsed.get("status") == "PASSED" and not missing_criticals else "FAILED",
            ...
            return AIReviewReport(step_template.step_number, "FAILED", 0.0, [f"Error: {e}"], [], datetime.now(timezone.utc).isoformat())

    def _mock_llm_api_call(self, payload: str, static_passed: bool) -> str:
        """
        Mock API to simulate AI validation behavior.
        """
        if not static_passed:
            return json.dumps({
                "status": "FAILED",
                "score": 40.0,
                "findings": ["Static analysis pre-check failed. Critical logic appears missing."],
                "missing_elements": []
            })
        return json.dumps({
            "status": "PASSED",
            "score": 95.0,
            "findings": ["Vectorization detected.", "Logic looks sound."],
            "missing_elements": []
        })


def run_full_validation_suite(code_files_map: Dict[int, str]) -> List[AIReviewReport]:
    """
    Takes a dictionary of codebases mapping to each step and runs the AI Agent over them.
    """
    auditor = GravityAIAuditor()
    final_results = []

    print("\n" + "="*60)
    print(" GRAVITY AI - ALGORITHMIC STRATEGY VERIFICATION PROTOCOL")
    print("="*60 + "\n")

    for step_prompt in ALL_PROMPTS:
        # Fetch the specific code intended for this step
        target_code = code_files_map.get(step_prompt.step_number, "# NO CODE PROVIDED")

        # Run the agent evaluation
        report = auditor.analyze_step(step_prompt, target_code)
        final_results.append(report)

        # Display formatted output
        status_color = "🟢" if report.status == "PASSED" else "🔴"
        print(f"{status_color} Step {report.step_number} | Score: {report.score} | Status: {report.status}")
...
if __name__ == "__main__":
    # Example Dummy Data to trigger the logic.
    # In practice, you read your local Python modules and map them to the steps.
    dummy_code_map = {
        1: """
import pandas as pd
import numpy as np
...
# Vectorized operation check:
df = pd.DataFrame({'close': [100.0, 101.0, 102.0]})
df['log_ret'] = np.log(df['close'] / df['close'].shift(1))
df['direction'] = df['log_ret'].apply(lambda x: 1 if x > 0 else 0)
df['signal'] = np.where(df['log_ret'] > 0.005, 1, 0)
...
def calculate_indicators(df):
    ...
    df['truerange'] = np.maximum(tr1, np.maximum(tr2, tr3))
    df['atr'] = df['truerange'].rolling(window=14).mean()

    # Momentum confirmations
    df['buy_signal'] = (df['macd'] > df['macd_signal']) & (df['rsi'] < 30)
    return df
""",
        3: """
import numpy as np
...
# 1. Black-Scholes Formula for Option Pricing and Greeks
def black_scholes_call_greeks(S, K, T, r, sigma):
    # S: stock price, K: strike price, T: time to expiration (years), r: risk-free rate, sigma: volatility
    d1 = (np.log(S / K) + (r + 0.5 * sigma**2) * T) / (sigma * np.sqrt(T))
    d2 = d1 - sigma * np.sqrt(T)

    # Correct Black-Scholes call formula
...
def calculate_ivr(iv_series: pd.Series) -> float:
    # 52-week rolling high and low
    rolling_low = iv_series.rolling(window=252).min()
    rolling_high = iv_series.rolling(window=252).max()
    current_iv = iv_series.iloc[-1]
    low_iv = rolling_low.iloc[-1]
    high_iv = rolling_high.iloc[-1]

    ivr = (current_iv - low_iv) / (high_iv - low_iv + 1e-10) * 100.0
    return ivr

# 3. Automated Options Strategy Matrix based on IVR
def route_options_strategy(ivr: float) -> str:
    if ivr > 70:
        return "Credit Spread"  # Neutral/Bearish or Bullish Premium Selling
    elif ivr < 30:
        return "Debit Spread"   # Premium Buying
    else:
        return "Iron Condor"
""",
        4: """
...
# 1. Stationarity Checks via ADF
def check_and_transform_stationarity(series: pd.Series) -> pd.Series:
    result = adfuller(series.dropna())
    p_value = result[1]
    if p_value > 0.05:
        # Non-stationary, use log returns or differences
        diff_series = np.log(series / series.shift(1)).dropna()
        # Or standard difference:
        # diff_series = series.diff().dropna()
        return diff_series
    return series

# 2. Scaling & Reshaping for LSTM Models
def prepare_lstm_data(data: pd.DataFrame, time_steps: int = 10):
    scaler = MinMaxScaler()
    ...
    for i in range(time_steps, len(scaled_data)):
        ...
        y.append(scaled_data[i, 0]) # target closing price

    X_arr, y_arr = np.array(X), np.array(y)
    # Reshaping to 3D tensor: [samples, time_steps, features]
    X_3d = X_arr.reshape((X_arr.shape, X_arr.shape[1], X_arr.shape[2]))
    return X_3d, y_arr
""",
        5: """
import pandas as pd
...
# 1. Macro Regime Ingestion via FRED API
def fetch_fred_macro_data(api_key: str) -> MacroEconomicDTO:
    fred = Fred(api_key=api_key)
    # 10Y-2Y Yield Curve, Sahm Rule, and High Yield Credit Spreads
...
# 2. Fundamental Valuation & Regime Governance (RISK OFF override)
def evaluate_market_regime_and_fundamentals(dto: FundamentalDataDTO, macro: MacroEconomicDTO):
    pe = dto.pe_ratio # P/E Ratio check
    div_yield = dto.dividend_yield

    # Intrinsic value Graham check
    intrinsic_val = dto.graham_number

    # Regime Governance Override
    # If Yield Curve is inverted AND Credit Spreads are widening, force "RISK OFF"
    if macro.yield_curve < 0.0 and macro.credit_spread > 4.5:
        regime = "RISK OFF"
    else:
        regime = macro.market_regime

    return regime, intrinsic_val
""",
        6: """
import numpy as np
import pandas as pd

# 1. MAE & MFE Tracking (Maximum Adverse/Favorable Excursion)
def calculate_mae_mfe(entry_price: float, high_series: pd.Series, low_series: pd.Series, position_type='long') -> tuple:
    if position_type == 'long':
        mae = (low_series.min() - entry_price) / entry_price # Maximum Adverse Excursion
        mfe = (high_series.max() - entry_price) / entry_price
    else:
        mae = (entry_price - high_series.max()) / entry_price # Maximum Adverse Excursion
        mfe = (entry_price - low_series.min()) / entry_price
    return mae, mfe

# 2. Dynamic ATR Position Sizing & Portfolio Heat Check
def compute_position_size(account_value: float, risk_percent: float, atr: float, multiplier: float, price: float, current_heat: float, max_heat: float = 0.06):
    if current_heat >= max_heat:
        return 0.0
    risk_amount = account_value * risk_percent
    stop_distance = atr * multiplier
    shares = risk_amount / stop_distance
    # Ensure position doesn't breach global portfolio heat
    return min(shares, (account_value * max_heat) / price)
"""
    }

    results = run_full_validation_suite(dummy_code_map)

    # Output final JSON summary string just like a real pipeline log
    report_json_str = json.dumps([asdict(r) for r in results], indent=4)
    report_filename = "Gravity_Verification_Report.json"
    try:
    ...
    try:
        ...
        print(f"\n✅ Verification cycle complete. Report saved to '{report_filename}'.")
    except Exception as e:
        print(f"\n⚠️ Warning: Could not write directly to '{report_filename}' due to file locking/permissions: {e}")
        alt_filename = "Gravity_Verification_Report_alt.json"
        try:
            with open(alt_filename, "w") as f:
                f.write(report_json_str)
            print(f"✅ Alternative verification report saved to '{alt_filename}'.")
        except Exception as alt_e:
            print(f"❌ Failed to write alternative report file: {alt_e}")# ==============================================================================
...
FRED_KEY = "<REDACTED_LEAKED_FRED_KEY_ROTATE_ME>"
...
def test_technical_options_engine_garch_volatility_and_ivr():
    ...
    # Calculate IVR using this volatility
    ivr = engine.calculate_ivr(df, vol)
    assert 0.0 <= ivr <= 100.0

def test_technical_options_engine_strategy_matrix():
    """
    Validates Automated Option Strategy Matrix logic mapping IVR and Trend to strategies.
    """
    from technical_options_engine import TechnicalOptionsEngine
    ...
    s1 = engine.generate_option_strategy_matrix(ivr=75.0, aroon_osc=50.0, coppock_val=0.5)
    assert "Put Credit Spread" in s1
    
    # High IVR + Bearish Trend
    s2 = engine.generate_option_strategy_matrix(ivr=75.0, aroon_osc=-30.0, coppock_val=-0.2)
    assert "Call Credit Spread" in s2
    
    # High IVR + Neutral Trend
    s3 = engine.generate_option_strategy_matrix(ivr=80.0, aroon_osc=10.0, coppock_val=-0.1)
    assert "Iron Condor" in s3
    
    # Low IVR + Bullish Trend
    s4 = engine.generate_option_strategy_matrix(ivr=25.0, aroon_osc=40.0, coppock_val=0.3)
    assert "Call Debit Spread" in s4
    
    # Low IVR + Bearish Trend
    s5 = engine.generate_option_strategy_matrix(ivr=10.0, aroon_osc=-45.0, coppock_val=-0.5)
    assert "Put Debit Spread" in s5
    
    # Neutral IVR + Bullish Trend
    s6 = engine.generate_option_strategy_matrix(ivr=50.0, aroon_osc=50.0, coppock_val=0.5)
...
def test_rsi_unadjusted_split_anomaly():
    ...
    """
    # 1. Create the Split Array: 15 days at $100, then a 2:1 split to $50
    pre_split = [100.0] * 15
    post_split = [50.0] * 15
...
def test_garch_and_edge_scoring():
    ...
    # Case A: Strong Edge, Low GARCH -> Max Kelly Size (25%)
    ...
    assert res_strong_edge["Kelly Target"] == 0.25 # Kelly sizing allowed 25% because Sortino > 1.0 and edge_ratio >= 1.0
    ...
    # Case D: Kelly sizing restriction on positive Edge Ratio
    ...
    assert res_restricted_kelly["Kelly Target"] == 0.15
