...
strategy_engine.py (Decision Maker: Institutional signal generator, calibrated momentum/trend weights, and tactical risk zone sizing. Utilizes an Aroon Oscillator Chop-Filter to suppress false-positive MACD signals. Position sizing ("Kelly Target") is now volatility-targeting + estimated-p fractional Kelly via the sizing/ package, not a score-derived formula)
