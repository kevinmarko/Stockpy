...
def _pr_cached_versions(reg, prompt_id: str) -> List[str]:
    ...
    try:
        ...
        logger.error(f"Failed to list versions for {prompt_id}: {e}", exc_info=True)
        return []
...
def _pr_all_known_ids(reg) -> List[str]:
    ...
    try:
        ...
        logger.error(f"Failed to get all known IDs: {e}", exc_info=True)
        return []
...
def run_backtest(symbol: str, period: str = "1y") -> str:
    ...
    try:
        ...
        # Resolve a real market cap so TieredCostModel's liquidity-tier cost
        # differentiation is actually exercised for this symbol, instead of always
        # defaulting to the "large_cap" tier. Never fabricated (CONSTRAINT #4/#6):
        # any fetch failure or non-finite/non-positive value degrades to None, which
        # run_backtrader_simulation treats identically to today's prior hardcoded behavior.
        market_cap = None
        try:
        ...
        for line in output_txt.splitlines():
            if "Starting Portfolio Value:" in line:
                try:
                    ...
                    logger.error(f"Failed to parse start_val: {e}", exc_info=True)
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            try:
                ...
                table_text = "\n".join(lines[:-1]).strip()
                out = f"# Validation Harness Results: {label}\n\n"
                if table_text:
                    out += f"{table_text}\n\n"
                out += f"```json\n{json_line}\n```"
                return out
            except json.JSONDecodeError:
                # Not JSON, return unfenced
                return f"# Validation Harness Results: {label}\n\n{stdout_clean}"
        return (
    ...
    if settings.BROWSER_DIAGNOSTICS_ENABLED:
        ...
        if real.get("available"):
            payload = {
                ...
                "statusText": "OK",
                "responseTimeMs": real.get("response_time_ms", 0.0),
                "title": real.get("title", "Pilots PWA"),
                "domNodeCount": real.get("dom_node_count", 0),
                "scriptsLoaded": [],
                "consoleMessages": real.get("console_messages", []),
                "screenshotBase64": real.get("screenshot_base64"),
            }
            lines = [
                f"# DevTools Screen Inspection: `{route}`\n",
                f"- **URL**: {url}",
                f"- **Status**: 🟢 {payload['status']} {payload['statusText']}",
                f"- **Response Time**: {payload['responseTimeMs']} ms",
            ...
            return "\n".join(lines)
        # Real capture unavailable -- fall through to the HTTP-reachability-only
        # path below rather than failing the tool call.
...
def audit_webapp_vitals(route: str = "/") -> str:
    ...
    # No real Lighthouse/CDP audit capability available on this path -- degrade
    # to None (unmeasured) rather than a fabricated plausible-looking vital.
    # Same key names/shape as the real-capture path above (ttfb_ms/fcp_ms/
    # lcp_ms/cls, plus vitals_rating) so the widget renders both through one
    # code path instead of needing to know which path produced the payload.
    scores = {
        "performance": None,
        "accessibility": None,
        "bestPractices": None,
    ...
    vitals = {
        "ttfb_ms": ttfb_ms if is_online else None,
        "fcp_ms": None,
        "lcp_ms": None,
        "cls": None,
    }
    payload = {
        ...
        "scores": scores,
        "vitals": vitals,
        "vitals_rating": {"ttfb": None, "fcp": None, "lcp": None, "cls": None},
        "timestamp": time.strftime("%Y-%m-%d %H:%M:%SZ", time.gmtime()),
    }
...
def compare_screen_snapshots(route: str = "/", threshold_pct: float = 1.0) -> str:
    """
    Compares a live PWA screen render against a saved golden-snapshot baseline.
    With settings.BROWSER_DIAGNOSTICS_ENABLED=True (requires the optional
    `playwright` package -- see requirements-optional.txt), this takes a REAL
    headless-Chromium screenshot and does a real pixel-diff (Pillow + numpy)
    against a baseline saved under output/visual_baselines/ -- the first call
    for a route establishes that baseline rather than fabricating a match.
    Without it, this degrades to an honest reachability-only check (no image
    ...
    if settings.BROWSER_DIAGNOSTICS_ENABLED:
        ...
        real = browser_diagnostics.capture_page_diagnostics(
            target_url, timeout_seconds=settings.BROWSER_DIAGNOSTICS_TIMEOUT_SECONDS
        )
        if real.get("available") and real.get("screenshot_base64"):
            ...
            if diff.get("available"):
                ...
                lines = [
                    ...
                    f"- **Diff Percentage**: {payload['diff_pct']:.2f}% (Threshold: {threshold_pct}%)",
                ...
                return "\n".join(lines)
        # Real capture/diff unavailable -- fall through to the
        # reachability-only path below rather than failing the tool call.
    ...
    # No real image comparison was performed on this path (no screenshot was
    # ever taken) -- "match"/"diff_pct" here are a reachability proxy, not a
    # real pixel diff; the JSON makes that explicit via the absence of
    # baseline_established/actualImg, and the markdown says so below.
    payload = {
        "route": route,
        "match": reachable,
