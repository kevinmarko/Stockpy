...
class SignalAggregator:
    ...
    def aggregate(
        ...
        for name, output in outputs.items():
            ...
            if meta_registry.has(name):
                ...
                # No MetaLabeler registered: fail-closed to 0.0 to prevent untrained 
                # models from passing live confidence gates at full confidence.
                mlp = 0.0
                if mlp < settings.META_LABEL_MIN_CONFIDENCE:
                    meta_hard_gate = True
                    mlp = 0.0
    ...
    def aggregate_vectorized(self, universe_df: pd.DataFrame, context: SignalContext) -> Dict[str, Tuple[float, List[str], List[str], List[str], Dict[str, SignalOutput], float]]:
        ...
        for name, df_out in outputs.items():
            ...
            mlp_raw = df_out.get('meta_label_proba', pd.Series(0.0, index=universe_df.index)).astype(float)
            
            # If the column wasn't generated by meta_registry (meaning no model), it defaults to 0.0.
            if not meta_registry.has(name):
                mlp_raw[:] = 0.0

            clamped_mlp_columns[name] = mlp_raw.clip(lower=0.0, upper=1.0)
