...
The Streamlit wiring lives in :func:`legacy.streamlit_command_center.panels.render_ai_control_center`.
This module hosts the pure logic it depends on so it is unit-testable WITHOUT
Streamlit — mirrors :mod:`shared.ai_insights_panel` / :mod:`shared.gravity_ai_panel`.
...
* :func:`validate_toggle_write` — guards a toggle write: the key must be in
  ``shared.env_io.ALLOWED_KEYS`` and must NOT be a secret (CONSTRAINT #3).
...
# time. Writing one of these via ``shared.env_io.write_setting`` and then
...
def validate_toggle_write(key: str) -> None:
    ...
    * A secret key (in ``shared.env_io.SECRET_KEYS``) raises ``SecretWriteError``.
    * A key outside ``shared.env_io.ALLOWED_KEYS`` raises ``DisallowedKeyError``.

    The actual write still goes through :func:`shared.env_io.write_setting` (which
    re-checks); this is a pre-flight guard so the panel can refuse a bad toggle
    before touching ``.env``.
    """
    from shared.env_io import ALLOWED_KEYS, SECRET_KEYS, DisallowedKeyError, SecretWriteError  # noqa: PLC0415
