...
class RunHandle:
    ...
    *   ``"daemon"`` — a cycle TRIGGERED over HTTP against an
        ...
        :mod:`shared.daemon_client` instead of a ``Popen`` handle. Do not
...
def _daemon_run_status(daemon_run_id: Optional[str]) -> Optional[dict]:
    """Look up a daemon-hosted run's status via shared.daemon_client, never
    ...
    if not daemon_run_id:
        return None
    try:
        from shared.daemon_client import get_run_status
...
def launch_orchestrator(dry_run: bool = False, refresh_account: bool = False) -> RunHandle:
    ...
    cycle over HTTP via :mod:`shared.daemon_client` against the running daemon
    ...
    if settings.ORCHESTRATOR_DAEMON_ENABLED and not dry_run:
        try:
            from shared.daemon_client import daemon_available, trigger_run
...
def read_log_tail(max_lines: int = 200, handle: Optional[RunHandle] = None) -> str:
    ...
    synthesized from :mod:`shared.daemon_client` instead.
    """
...
class RunProgress:
    ...
    percent:
        ...
        difference (see :func:`legacy.streamlit_command_center.panels.launcher._render_live_status_body`).
    label:
