...
env_io.py
=========
Safe, allowlist-bounded read/write layer for the project ``.env`` file.

Relocated from ``gui/env_io.py`` to the repo root (F13, docs/
module_efficiency_redundancy_audit.md): this module is a safety-critical
SECRET_KEYS/ALLOWED_KEYS credential-write registry, imported directly by
production API code entirely outside the decommissioned ``gui/`` package
(``api/pilots_api.py``, ``api/_redact.py``, ``api/data_api.py``,
``investyo_mcp_server.py``, ``runtime_flags_writer.py``, and others) —
living inside a frozen, no-new-development package was an architectural
inversion for code the live backend genuinely depends on. ``gui/env_io.py``
is now a thin re-export shim so the frozen Command Center's own internal
imports (``from gui import env_io`` / ``from shared.env_io import X``) keep
working unchanged; see that file's own docstring.

Originally written for, and still used by, the Command Center's **Settings
Manager** and **Strategy Matrix** tabs.
...
# Repo root = this file's own directory (env_io.py lives at the repo root as
# of the F13 relocation above). ENV_PATH itself lives in settings.py — the
# single source of truth every `.env` locator in the codebase must import
# instead of re-deriving (see that module's comment for why three
# independent locators used to disagree with each other). Unused within
# this module today (kept correct rather than removed, in case a future
# caller needs it) -- was ``.parent.parent`` before the relocation, when
# this file lived one directory deeper at gui/env_io.py.
...
ALLOWED_KEYS: tuple[str, ...] = (
    ...
    # writing it through this module only ever updates .env, which is read
