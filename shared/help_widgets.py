...
gui/help_widgets.py
===================
Thin Streamlit wrappers over ``shared.help_content``.  Each widget degrades
...
``explain(tab_id, *, expanded=False)``
    Renders the full :class:`~shared.help_content.TabHelp` for a tab as a
...
from shared.help_content import get_tab_help, metric_help, section_help

logger = logging.getLogger(__name__)


def explain(tab_id: str, *, expanded: bool = False) -> None:
    """Render the :class:`~shared.help_content.TabHelp` for *tab_id* as a
...
def glossary_chip(term: str) -> None:
    ...
    """
    from shared.help_content import get_glossary  # local to avoid any circular-import risk
