"""Client for fetching Google Trends data with rate-limiting and overlapping windows."""
import logging
import threading
import time
from datetime import datetime, timedelta
from typing import List

import pandas as pd

from settings import settings

logger = logging.getLogger(__name__)

# Track consecutive failures for the cooldown (process-wide). All reads and
# writes of this shared state are guarded by `_state_lock`, held across every
# access -- including the min-interval sleep -- mirroring
# data/sentiment_sources.py's `_gdelt_throttle_lock` pattern (see
# `_gdelt_throttle`'s own docstring for why the lock spans the sleep).
# `time.monotonic()` (never `time.time()`) is used for every elapsed-time
# computation so a real-clock/NTP step can't make an elapsed gap go negative
# and skip a wait, or make the cooldown-until check misfire.
_state_lock = threading.Lock()
_consecutive_failures = 0
_cooldown_until = 0.0
_last_request_time = 0.0

def fetch_overlapping_windows(
    query_term: str,
    start_date: str,
    end_date: str,
    window_days: int = 90,
    overlap_days: int = 30
) -> List[pd.Series]:
    """
    Fetch Google Trends data for a query term over a date range, breaking the range
    into overlapping windows.

    Args:
        query_term: The search term to query.
        start_date: Start date string in 'YYYY-MM-DD' format.
        end_date: End date string in 'YYYY-MM-DD' format.
        window_days: The size of each fetch window in days.
        overlap_days: The number of days consecutive windows should overlap.

    Returns:
        A list of pandas Series objects containing the trend data for each window,
        or an empty list if a failure or cooldown prevents fetching.
    """
    global _consecutive_failures, _cooldown_until, _last_request_time

    if not settings.GOOGLE_TRENDS_ENABLED:
        logger.warning("Google Trends ASVI is disabled via settings.")
        return []

    if window_days <= overlap_days:
        logger.warning("window_days must be greater than overlap_days.")
        return []

    try:
        start_dt = datetime.strptime(start_date, "%Y-%m-%d")
        end_dt = datetime.strptime(end_date, "%Y-%m-%d")
    except (TypeError, ValueError) as exc:
        # CONSTRAINT #6: a malformed date string must degrade to an empty
        # result, never raise out to the caller, exactly like every other
        # failure mode in this function.
        logger.warning(
            "Google Trends fetch given an unparseable date (start=%r, end=%r): %s",
            start_date, end_date, exc,
        )
        return []

    if start_dt > end_dt:
        return []

    try:
        # Lazy import: pytrends is an OPTIONAL, strictly-supplementary
        # dependency (requirements-optional.txt only -- see that file's
        # comment) -- matching data/attention_sources.py's
        # `_fetch_pytrends_attention_score()` convention exactly, so this
        # module never crashes on import for an operator without pytrends
        # installed, regardless of GOOGLE_TRENDS_ENABLED.
        from pytrends.request import TrendReq
    except Exception as exc:
        logger.warning(
            "pytrends not installed / import failed (%s); Google Trends ASVI unavailable.",
            exc,
        )
        return []

    try:
        # `TrendReq.__init__` (in the real installed pytrends package)
        # unconditionally makes a real synchronous network call
        # (GetGoogleCookie), whose own exception handling only catches
        # requests.exceptions.ProxyError -- any other network failure
        # (timeout, DNS failure, a generic ConnectionError) would otherwise
        # propagate straight out of this function. CONSTRAINT #6: degrade to
        # an empty result instead, exactly like every other failure mode
        # here.
        pytrends = TrendReq(hl='en-US', tz=360)
    except Exception as exc:
        logger.warning("Failed to construct pytrends TrendReq client: %s", exc)
        return []

    results: List[pd.Series] = []

    current_start = start_dt

    while current_start < end_dt:
        current_end = current_start + timedelta(days=window_days)
        if current_end > end_dt:
            current_end = end_dt

        timeframe = f"{current_start.strftime('%Y-%m-%d')} {current_end.strftime('%Y-%m-%d')}"

        # Enforce shared rate limiter / cooldown.
        in_cooldown = False
        cooldown_remaining = 0.0
        with _state_lock:
            now = time.monotonic()
            if settings.GOOGLE_TRENDS_COOLDOWN_THRESHOLD > 0 and _consecutive_failures >= settings.GOOGLE_TRENDS_COOLDOWN_THRESHOLD:
                if now < _cooldown_until:
                    in_cooldown = True
                    cooldown_remaining = _cooldown_until - now
                else:
                    # Cooldown expired, tentatively reset
                    _consecutive_failures = 0

        if in_cooldown:
            logger.warning(
                "Google Trends client is in cooldown for another %.0fs. Skipping fetch for %s.",
                cooldown_remaining,
                query_term
            )
            return []

        # Enforce minimum request interval. The lock is held across the
        # sleep itself (not just the read/write around it) -- see the
        # module-level comment above for why.
        if settings.GOOGLE_TRENDS_MIN_REQUEST_INTERVAL_SECONDS > 0:
            with _state_lock:
                elapsed = time.monotonic() - _last_request_time
                if elapsed < settings.GOOGLE_TRENDS_MIN_REQUEST_INTERVAL_SECONDS:
                    time.sleep(settings.GOOGLE_TRENDS_MIN_REQUEST_INTERVAL_SECONDS - elapsed)
                _last_request_time = time.monotonic()

        attempt = 0
        success = False
        series: pd.Series = pd.Series(dtype='float64')
        max_retries = max(0, settings.GOOGLE_TRENDS_MAX_RETRIES)

        while attempt <= max_retries and not success:
            with _state_lock:
                _last_request_time = time.monotonic()
            try:
                pytrends.build_payload(kw_list=[query_term], timeframe=timeframe)
                df = pytrends.interest_over_time()

                # Check if empty
                if df.empty or query_term not in df.columns:
                    # Sometimes empty responses happen if no data
                    logger.warning("Google Trends returned empty data for %s during %s", query_term, timeframe)
                    break # Not a failure per se, just no data, so break retry loop

                series = df[query_term]
                success = True
                with _state_lock:
                    _consecutive_failures = 0 # reset on success

            except Exception as e: # Catch all, including requests and pytrends exceptions
                attempt += 1
                cooldown_triggered = False
                with _state_lock:
                    _consecutive_failures += 1
                    # Apply cooldown if threshold hit
                    if settings.GOOGLE_TRENDS_COOLDOWN_THRESHOLD > 0 and _consecutive_failures >= settings.GOOGLE_TRENDS_COOLDOWN_THRESHOLD:
                        _cooldown_until = time.monotonic() + settings.GOOGLE_TRENDS_COOLDOWN_SECONDS
                        cooldown_triggered = True

                logger.warning(
                    "Google Trends request failed (attempt %d/%d) for %s: %s",
                    attempt, max_retries + 1, query_term, e
                )

                if cooldown_triggered:
                    logger.warning("Google Trends cooldown threshold reached. Cooldown activated.")
                    return []

                if attempt <= max_retries:
                    # Simple backoff (pytrends doesn't expose headers easily, so we just do exponential)
                    time.sleep(5.0 * (2 ** (attempt - 1)))

        if success:
            results.append(series)

        # Advance the window
        current_start = current_end - timedelta(days=overlap_days)
        # Prevent infinite loop if overlap >= window_days, handled by check above
        if current_end >= end_dt:
            break

    return results

def reset_limiter_state():
    """Reset the rate limiter state (useful for tests)."""
    global _consecutive_failures, _cooldown_until, _last_request_time
    with _state_lock:
        _consecutive_failures = 0
        _cooldown_until = 0.0
        _last_request_time = 0.0
