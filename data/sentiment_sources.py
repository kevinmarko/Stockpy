...
context window), not just the always-on 8-K submissions-feed headlines. See
its class docstring for the form-filtering, chunking, and shared-throttle
details.
...
class EdgarSource(SentimentSource):
    ...
    * Every document is stamped off the filing's OFFICIAL ``file_date``
      ...
      happened to run (see the lookahead-safety test in
      ``tests/test_sentiment_sources.py``, mirroring
      ``tests/test_sentiment_pit_lookahead.py``'s style).
